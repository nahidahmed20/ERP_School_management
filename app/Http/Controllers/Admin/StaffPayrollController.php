...
class StaffPayrollController extends Controller
{
    public function generate(Request $request, PayrollService $service)
    {
        $data = $request->validate([
            ...
            'staff_id' => ['nullable', CampusRule::exists('staff')],
            'working_days' => 'nullable|integer|min:1|max:31',
            'overtime_enabled' => 'required|boolean',
            'overtime_mode' => 'required_if:overtime_enabled,true|in:hour,day',
    ...
    public function attendanceAdjustment(Request $request, StaffAttendance $attendance)
    {
        ...
        return back()->with('success', 'Attendance payroll adjustment saved.');
    }
    ...
    {
        $payroll = StaffPayroll::findOrFail($id);
        abort_unless(($payroll->approval_status ?: 'draft') === 'draft', 422, 'Only draft payroll can be deleted.');
        $payroll->delete();
