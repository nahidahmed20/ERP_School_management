...
class LibraryOperationsController extends Controller
{
    ...
    public function master(Request $r)
    {
        $d = $r->validate([
            'type' => 'required|in:authors,publishers,categories',
            'name' => 'required|string|max:255',
            'description' => 'nullable|string|max:1000'
        ]);

        $this->raw('library_' . $d['type'])->updateOrInsert(
            ['campus_id' => config('app.active_campus_id'), 'name' => $d['name']],
            ['description' => $d['description'] ?? null, 'is_active' => true, 'created_at' => now(), 'updated_at' => now()]
        );

        return back()->with('success', 'Library master saved.');
    }
    ...
    public function copy(Request $r)
    {
        $d = $r->validate([
            ...
            'accession_no' => 'required|string|max:100|unique:book_copies',
            'barcode' => 'nullable|string|max:100|unique:book_copies',
        ...
        return back()->with('success', 'Book accession/copy added.');
    }
    ...
    public function member(Request $r)
    {
        $d = $r->validate([
            ...
            'card_no' => 'required|string|max:100|unique:library_members',
            'barcode' => 'nullable|string|max:100|unique:library_members',
    ...
    public function scan(Request $r)
    {
        ...
        return response()->json(['copy' => $copy, 'member' => $member]);
    }
    ...
    public function returnBook(Request $r, BookIssue $issue)
    {
        $d = $r->validate([
            'condition_on_return' => 'required|in:good,fair,damaged,lost',
            'replacement_charge' => 'nullable|numeric|min:0'
        ]);

        DB::transaction(function () use ($issue, $d) {
            $copy = BookCopy::lockForUpdate()->find($issue->book_copy_id);
            $lost = $d['condition_on_return'] === 'lost';
            $damaged = $d['condition_on_return'] === 'damaged';

            // Overdue Fine Calculation (10 BDT per day)
            $fineAmount = 0;
            $dueDate = Carbon::parse($issue->due_date)->endOfDay();
            if (now()->isAfter($dueDate)) {
                $fineAmount = now()->diffInDays($dueDate) * 10;
            }

            $issue->update([
                'return_date' => today(),
                'condition_on_return' => $d['condition_on_return'],
                'replacement_charge' => $d['replacement_charge'] ?? 0,
                'fine_amount' => $issue->fine_amount + $fineAmount,
                'status' => $lost ? 'Lost' : 'Returned'
            ]);

            if ($copy) {
                $copy->update([
                    'status' => $lost ? 'lost' : ($damaged ? 'damaged' : 'available'),
                    'condition' => $d['condition_on_return']
                ]);
                if (!$lost && !$damaged) {
                    $copy->book()->increment('available');
                }
            }
        });

        return back()->with('success', 'Return processing completed. Overdue fine calculated automatically.');
    }

    public function reservation(Request $r, LibraryReservation $reservation)
    {
        $d = $r->validate([
            'status' => 'required|in:Approved,Rejected,Fulfilled,Cancelled',
            'note' => 'nullable|string|max:1000'
        ]);

        if ($d['status'] === 'Approved') {
            abort_unless(BookCopy::where('book_id', $reservation->book_id)->where('status', 'available')->exists(), 422, 'No copy is currently available.');
        }

        $reservation->update($d + [
            'expires_at' => $d['status'] === 'Approved' ? today()->addDays(3) : $reservation->expires_at
        ]);

        return back()->with('success', 'Reservation updated.');
    }

    public function finePayment(Request $r, BookIssue $issue)
    {
        $d = $r->validate([
            'amount' => 'required|numeric|min:0.01',
            'payment_method' => 'required|in:Cash,Bank,Mobile Banking,Online',
            'transaction_id' => 'nullable|string|max:255'
        ]);

        $due = max(0, $issue->fine_amount + $issue->replacement_charge - $issue->fine_paid);
        abort_if($d['amount'] > $due, 422, 'Payment exceeds outstanding library charge.');

        DB::transaction(function () use ($issue, $d, $r) {
            LibraryFinePayment::create($d + [
                'campus_id' => config('app.active_campus_id'),
                'book_issue_id' => $issue->id,
                'received_by' => $r->user()->id,
                'paid_at' => now()
            ]);
            $issue->increment('fine_paid', $d['amount']);
        });

        return back()->with('success', 'Library fine/charge payment received.');
    }

    public function startStock(Request $r)
    {
        $id = DB::table('library_stock_checks')->insertGetId([
            'campus_id' => config('app.active_campus_id'),
            'reference_no' => 'STOCK-' . now()->format('YmdHis'),
            'status' => 'open',
        ...
        return back()->with('success', 'Physical stock check started: #' . $id);
    }
    ...
    public function scanStock(Request $r, int $check)
    {
        ...
        return back()->with('success', 'Copy counted.');
    }
