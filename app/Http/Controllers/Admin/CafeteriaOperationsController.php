...
class CafeteriaOperationsController extends Controller
{
    ...
    {
        $wallets = CafeteriaWallet::with('user:id,name,email')->latest()->take(250)->get();
        $orders = CafeteriaOrder::with(['customer:id,name', 'outlet:id,name'])->latest()->take(150)->get();
        $today = today();
        $served = CafeteriaOrder::whereDate('created_at', $today)->whereNotIn('status', ['Voided', 'Cancelled']);
        $consumption = $orders->whereNotIn('status', ['Voided', 'Cancelled'])->flatMap(fn ($o) => collect($o->items)->map(fn ($i) => ['name' => $i['name'] ?? 'Item', 'quantity' => (int) ($i['quantity'] ?? 0), 'amount' => (float) ($i['subtotal'] ?? 0)]))->groupBy('name')->map(fn ($rows, $name) => ['name' => $name, 'quantity' => $rows->sum('quantity'), 'amount' => $rows->sum('amount')])->sortByDesc('quantity')->values();

        return Inertia::render('Admin/CafeteriaOperations/Index', ['wallets' => $wallets, 'users' => User::with(['student:id,user_id,admission_no,first_name,last_name', 'staff:id,user_id,staff_id_no,first_name,last_name'])->orderBy('name')->get(['id', 'name', 'email']), 'outlets' => CafeteriaOutlet::where('is_active', true)->get(['id', 'name']), 'foods' => FoodItem::with('outlet:id,name')->orderBy('name')->get(), 'purchaseItems' => PurchaseItem::where('is_active', true)->orderBy('name')->get(['id', 'name', 'quantity', 'reorder_level']), 'orders' => $orders, 'refunds' => $this->raw('cafeteria_refund_requests')->latest()->take(100)->get(), 'closings' => $this->raw('cafeteria_cash_closings')->latest('business_date')->take(50)->get(), 'consumption' => $consumption, 'summary' => ['sales' => (float) (clone $served)->sum('total_amount'), 'orders' => (clone $served)->count(), 'walletBalance' => (float) CafeteriaWallet::sum('balance'), 'lowStock' => FoodItem::whereColumn('stock_quantity', '<=', 'reorder_level')->count(), 'kitchenPending' => CafeteriaOrder::whereIn('status', ['Pending', 'Accepted', 'Preparing', 'Ready'])->count()]]);
    }
    ...
    public function purchase(Request $r, InventoryService $inventory)
    {
        ...
        $order = DB::transaction(function () use ($d, $r, $inventory) {
            ...
            foreach ($items as $line) {
                ...
                if ($food?->purchase_item_id) {
                    $stock = PurchaseItem::findOrFail($food->purchase_item_id);
                    $cost += (float) $stock->purchase_price * (int) $line['quantity'];
                    $inventory->move($stock, -$line['quantity'], 'cafeteria_consumption', $order, 'Cafeteria order '.$order->order_number);
                }
            }
            ...
            $accounting->post('cafeteria-cogs:'.$order->id, $order, '5000', '1200', $cost, 'Cafeteria consumption '.$order->order_number, now()->toDateString());
    ...
    public function refundDecision(Request $r, int $refund, InventoryService $inventory)
    {
        $d = $r->validate(['status' => 'required|in:approved,rejected']);
        DB::transaction(function () use ($d, $refund, $r, $inventory) {
            ...
            if ($d['status'] === 'approved') {
                ...
                if ($order->payment_method === 'wallet') {
                    ...
                    foreach ($order->items as $line) {
                        ...
                        if ($food?->purchase_item_id) {
                            $inventory->move(PurchaseItem::findOrFail($food->purchase_item_id), (int) $line['quantity'], 'cafeteria_void_return', $order, 'Voided cafeteria order '.$order->order_number);
                        }
    ...
    {
        $d = $r->validate(['purchase_item_id' => ['nullable', CampusRule::exists('purchase_items')], 'stock_quantity' => 'required|numeric|min:0', 'reorder_level' => 'required|numeric|min:0', 'stock_unit' => 'required|string|max:30']);
        $food->update($d);

        return back()->with('success', 'Food stock and purchase-item link updated.');
    }
