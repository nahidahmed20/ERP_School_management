...
class ExamScheduleController extends Controller
{
    public function index(Request $request)
    {
        ...
        return Inertia::render('Admin/Exams/Schedule/Index', [
            'schedules' => $query->get(),
            'exams' => Exam::where('is_active', true)->orderBy('start_date', 'desc')->get(),
    ...
    public function store(Request $request)
    {
        ...
        return back()->with([
            'success' => 'নতুন পরীক্ষা তৈরি করা হয়েছে। এবার রুটিন সেট করুন।',
            'new_exam_id' => $exam->id 
        ]);
    }
    ...
    public function bulkUpdate(Request $request)
    {
        $data = $request->validate([
            ...
            'periods.*.subject_id' => ['required', $this->existsRule('subjects'), 'distinct'],
        ...
        DB::transaction(function () use ($data, $class) {
            ...
            ExamSchedule::where('exam_id', $exam->id)
            ...
            foreach ($data['periods'] as $index => $period) {
                ...
                $clash = ExamSchedule::where('exam_date', $period['exam_date'])
                    ...
                    ->where(function ($query) use ($class, $data, $period) {
                        ...
                        if (! empty($period['classroom_id'])) {
                            $query->orWhere('classroom_id', $period['classroom_id']);
                        }
                    })->exists();
                ...
                ExamSchedule::create([
                    'campus_id' => config('app.active_campus_id') ?? $class->campus_id, 
                    'exam_id' => $exam->id, 
                    'class_id' => $class->id,
