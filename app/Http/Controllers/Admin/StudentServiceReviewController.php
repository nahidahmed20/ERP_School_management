...
class StudentServiceReviewController extends Controller
{
    ...
    public function homework(Request $request, HomeworkSubmission $submission)
    {
        ...
        $submission->update($data + ['evaluated_at' => now()]);
        return back()->with('success', 'Homework reviewed.');
    }
            ...
            if ($data['status'] === 'Approved') {
                ...
                foreach (CarbonPeriod::create($leave->start_date, $leave->end_date) as $day) {
                    ...
                    StudentAttendance::updateOrCreate(['student_id' => $student->id, 'attendance_date' => $date], [
                        ...
                        'recorded_by' => auth()->id(),
                        'verified_at' => now(),
                        'remarks' => 'Approved student leave #' . $leave->id,
                    ]);
                }
            }
    ...
    public function profile(Request $request, StudentProfileUpdateRequest $profile)
    {
        ...
        DB::transaction(function () use ($profile, $data) {
            $profile->update($data + ['reviewed_by' => auth()->id()]);
            if ($data['status'] === 'Approved') $profile->student->update(collect($profile->changes)->only(['phone', 'email', 'present_address', 'permanent_address'])->all());
        });
    ...
    {
        $data = $request->validate(['status' => 'required|in:Approved,Rejected,Fulfilled,Cancelled', 'note' => 'nullable|string|max:1000']);
