...
export default function SiteLayout({ children, activePage = "" }) {
    ...
    const initial = (settings.school_short_name ||
    ...
    return (
        <>
            ...
            <div
                className="sf-site"
                style={{
                    "--sf-primary": settings.primary_color || "#12372A",
                    "--sf-accent": settings.accent_color || "#E9B949",
                }}
            >
                <div className="sf-announcement">
                    <span>
                        Admissions {settings.admission_session || "Open"}
                    </span>
                    {settings.admission_deadline && (
                        <b>
                            Application deadline: {settings.admission_deadline}
                        </b>
                    )}
                    <Link href={route("site.admissions")}>
                        Apply now &rarr;
                    </Link>
                </div>
                <header className="sf-header">
                    <div className="sf-shell sf-nav">
                        <Brand />
                        <nav className={open ? "is-open" : ""}>
                            {nav.map(([key, label, url]) => (
                                <Link
                                    key={key}
                                    href={url}
                                    className={
                            ...
                            <Link href={route("login")} className="sf-login">
                                Portal login
                            </Link>
                        </nav>
                        <button
                ...
                {(flash.success || flash.error) && (
                    <div
                        className={`sf-flash ${flash.error ? "is-error" : ""}`}
                    >
                        {flash.success || flash.error}
                    </div>
                )}
                ...
                <footer className="sf-footer">
                    <div className="sf-shell sf-footer-grid">
                        ...
                        <div>
                            <h4>Portals</h4>
                            <Link href={route("login")}>Student portal</Link>
                            <Link href={route("login")}>Guardian portal</Link>
                            <Link href={route("login")}>Staff portal</Link>
                        </div>
                        <div>
                            <h4>Contact</h4>
                    ...
                    <div className="sf-shell sf-copyright">
                        &copy; {new Date().getFullYear()} {settings.school_name}
                        . {settings.copyright_text}
                        <span>{settings.powered_by_text}</span>
                    </div>
                </footer>
            </div>
        </>
