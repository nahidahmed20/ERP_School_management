...
export default function Index({ rooms, campuses, activeCampusId, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.medical.rooms.index'), { 
  ...
  const handleStatusToggle = (item) => {
    router.put(route('admin.medical.rooms.update', item.id), {
      ...
      preserveState: true,
      onSuccess: () => {
        Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Room status updated!', showConfirmButton: false, timer: 2000 });
      }
    });
  };
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Room No', 'Nurse / Doctor', 'Phone', 'Total Beds', 'Status'];
    ...
    link.setAttribute("download", `Medical_Rooms_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Room No\tNurse / Doctor\tPhone\tTotal Beds\tStatus\n";
    rooms.data.forEach(item => {
      text += `${item.room_number}\t${item.nurse_name || '-'}\t${item.phone || '-'}\t${item.total_beds}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Medical Rooms Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Rooms &amp; Staff</h1>
            <p className="text-sm text-slate-500 mt-1">ক্যাম্পাসের মেডিকেল রুম, বেড সংখ্যা এবং ডাক্তার বা নার্সের তথ্য পরিচালনা করুন।</p>
          </div>
          <button
        ...
        <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            ...
            <div className="relative flex-1 min-w-[200px] sm:w-80">
              ...
              <input
                type="text"
                placeholder="Search Room or Nurse..."
                value={search}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Nurse / Doctor</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Phone</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Beds</th>
                ...
                {rooms.data.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">No medical rooms found</p>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-bold text-slate-900 font-mono block">{item.room_number}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-800">{item.nurse_name}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setIsModalOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Room">
      ...
      {deletingItem && (
        <ConfirmDeleteModal 
          item={{ name: `Room ${deletingItem.room_number}` }} 
