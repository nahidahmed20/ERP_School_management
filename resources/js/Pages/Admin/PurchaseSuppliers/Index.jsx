...
export default function Index({ suppliers, filters }) {
  ...
  const applyFilters = (overrides = {}) => {
    router.get(route('admin.purchase.suppliers.index'), {
  ...
  const exportToCSV = () => {
    if (!suppliers.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Company Name', 'Contact Person', 'Phone', 'Email', 'Status'];
    const rows = suppliers.data.map(item => [
      item.name || 'N/A',
                ...
                {suppliers.data.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or add a new supplier</p>
                    </td>
                  </tr>
                ) : (
                  suppliers.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm font-semibold text-slate-700">{item.contact_person || '—'}</span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-sm font-bold text-slate-800 font-mono">{item.phone}</div>
                        <div className="text-xs text-slate-500 font-medium mt-0.5">{item.email || 'N/A'}</div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Supplier">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeleteId(item.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Supplier">
