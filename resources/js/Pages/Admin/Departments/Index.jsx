...
export default function Index({ departments, campuses, filters }) {
  ...
  const exportToCSV = () => {
    if (!departments.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Department Name', 'Description', 'Status'];
    const rows = departments.data.map(item => [
      item.name || 'N/A', 
  ...
  const copyToClipboard = () => {
    ...
    let text = "Department Name\tDescription\tStatus\n";
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Departments Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Departments</h1>
            <p className="text-sm text-slate-500 mt-1">স্টাফ ও শিক্ষকদের জন্য ডিপার্টমেন্ট পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Department
          </button>
        </div>
                ...
                {departments.data.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-slate-500">
                      কোনো ডিপার্টমেন্ট পাওয়া যায়নি।
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Department">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Department">
      ...
      {deletingItem && (
        <ConfirmDeleteModal
          ...
          onConfirm={() => {
            router.delete(route('admin.departments.destroy', deletingItem.id), {
