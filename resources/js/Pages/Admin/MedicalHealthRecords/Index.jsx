...
export default function Index({ records, users, campuses, activeCampusId, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.medical.health-records.index'), { 
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Student/Staff Name', 'Blood Group', 'Height', 'Weight', 'Emergency Contact', 'Allergies', 'Chronic Conditions'];
    const rows = records.data.map(item => [
      item.user?.name || 'N/A', 
      item.blood_group || 'N/A', 
      item.height || 'N/A', 
      item.weight || 'N/A', 
      item.emergency_contact || 'N/A',
      item.allergies || 'N/A',
      item.chronic_conditions || 'N/A'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Health_Records_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Name\tBlood Group\tHeight\tWeight\tEmergency Contact\n";
    records.data.forEach(item => {
      text += `${item.user?.name || '-'}\t${item.blood_group || '-'}\t${item.height || '-'}\t${item.weight || '-'}\t${item.emergency_contact || '-'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Health Records Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">শিক্ষার্থী এবং স্টাফদের প্রাথমিক স্বাস্থ্য তথ্য ও ব্লাড গ্রুপ পরিচালনা করুন।</p>
          </div>
          <button
        ...
        <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            ...
            <div className="relative flex-1 min-w-[200px] sm:w-80">
              ...
              <input
                type="text"
                placeholder="Search Student or Blood Group..."
                value={search}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Student / Staff Name</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Blood Group</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Height &amp; Weight</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Emergency Contact</th>
                ...
                {records.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">No health records found</p>
                    ...
                    <tr key={record.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-bold text-slate-900 block">{record.user?.name}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        {record.blood_group ? (
                          <span className="inline-flex px-2.5 py-1 rounded-md text-xs font-black bg-rose-50 text-rose-700 border border-rose-200 uppercase">
                            {record.blood_group}
                          </span>
                        ) : (
                      ...
                      <td className="px-6 py-4 text-sm font-medium text-slate-600">
                        <span className="text-slate-800">{record.height || '-'}</span> 
                        <span className="mx-2 text-slate-300">|</span> 
                        <span className="text-slate-800">{record.weight || '-'}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setViewingItem(record)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="View Record">
                            <Icon name="eye" className="w-4 h-4" />
                          </button>
                          <button onClick={() => { setEditingItem(record); setIsFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Record">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(record)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Record">
      ...
      {deletingItem && (
        <ConfirmDeleteModal 
          item={{ name: `Health Record of ${deletingItem.user?.name}` }} 
