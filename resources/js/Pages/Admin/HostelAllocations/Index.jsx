...
export default function Index({ allocations, rooms, users, campuses, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.hostel-allocations.index'), {
      search, room_id: roomId, status, per_page: perPage, ...overrides,
  ...
  const exportToCSV = () => {
    if (!allocations.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Occupant Name', 'Email', 'Hostel Name', 'Room No', 'Allocation Date', 'Monthly Fee', 'Status'];
    const rows = allocations.data.map(item => [
      item.user?.name || 'N/A',
      item.user?.email || 'N/A',
      item.room?.hostel_name || 'N/A',
      item.room?.room_number || 'N/A',
      item.allocation_date || 'N/A',
      item.monthly_fee || '0',
    ...
    link.setAttribute("download", `Room_Allocations_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!allocations.data.length) return;
    let text = "Occupant\tHostel\tRoom No\tDate\tMonthly Fee\tStatus\n";
    allocations.data.forEach(item => {
      text += `${item.user?.name}\t${item.room?.hostel_name}\t${item.room?.room_number}\t${item.allocation_date}\t${item.monthly_fee}\t${item.is_active ? 'Active' : 'Vacated'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Room Allocations Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <span className="text-xs font-bold tracking-wider text-indigo-600 uppercase">Campus Life</span>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Room Allocations</h1>
            <p className="text-sm text-slate-500 mt-1">স্টুডেন্ট বা স্টাফদের হোস্টেল রুম ও সিট বরাদ্দ করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> New Allocation
          </button>
        </div>
        ...
        <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            ...
            <select
              value={roomId}
              onChange={(e) => { setRoomId(e.target.value); applyFilters({ room_id: e.target.value }); }}
            ...
            >
              ...
              {rooms.map(r => <option key={r.id} value={r.id}>{r.hostel_name} - {r.room_number}</option>)}
            </select>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Occupant (User)</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Hostel &amp; Room</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Allocation Date</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Monthly Fee</th>
                ...
                {allocations.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">কোনো রুম বরাদ্দ পাওয়া যায়নি</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or create a new allocation</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm font-bold text-slate-900 block">{item.user?.name}</span>
                            <span className="text-xs text-slate-500 font-medium block mt-0.5">{item.user?.email}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-900 block">{item.room?.hostel_name}</span>
                        <span className="text-xs text-slate-500 font-mono block mt-0.5">Room No: {item.room?.room_number}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Allocation">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Allocation">
      ...
      {formOpen && <AllocationFormModal item={editingItem} rooms={rooms} users={users} campuses={campuses} activeCampusId={auth?.active_campus_id} onClose={() => setFormOpen(false)} />}
      {viewingItem && <AllocationShowModal item={viewingItem} onClose={() => setViewingItem(null)} />}

      {deletingItem && (
        <ConfirmDeleteModal
          item={{ name: `${deletingItem.user?.name}'s Room Allocation` }}
          onCancel={() => setDeletingItem(null)}
          onConfirm={() => {
            router.delete(route('admin.hostel-allocations.destroy', deletingItem.id), { onSuccess: () => setDeletingItem(null) });
          }}
        />
      )}
    </AuthenticatedLayout>
