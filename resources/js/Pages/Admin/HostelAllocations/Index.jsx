...
export default function Index({ allocations, rooms, users, campuses, filters }) {
  ...
  return (
      ...
      header={
        <div className="page-head">
          <div>
            ...
            <p className="desc">স্টুডেন্ট বা স্টাফদের হোস্টেল রুম ও সিট বরাদ্দ করুন।</p>
          </div>
    ...
    >
      ...
      <div className="card mm-card">
        <div className="mm-filters" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          <div className="search">
            <Icon name="search" />
            <input placeholder="Search user..." value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && applyFilters()} />
          </div>

          <select value={roomId} onChange={(e) => { setRoomId(e.target.value); applyFilters({ room_id: e.target.value }); }}>
          ...
          <table className="mm-table">
            ...
            <tbody>
              {allocations.data.length === 0 && (
                <tr><td colSpan={6} className="mm-empty">কোনো রুম বরাদ্দ পাওয়া যায়নি।</td></tr>
              )}
              {allocations.data.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <div style={{ fontWeight: 500 }}>{item.room?.hostel_name}</div>
                    <div style={{ fontSize: '0.75rem', color: '#6B7280' }}>Room No: {item.room?.room_number}</div>
                  </td>
                  <td>{item.allocation_date}</td>
                  <td><strong style={{ color: '#047857' }}>৳ {item.monthly_fee}</strong></td>
                  <td>
      ...
      {deletingItem && (
        <ConfirmDeleteModal item={{ name: deletingItem.user?.name + "'s Room Allocation" }} onCancel={() => setDeletingItem(null)} onConfirm={() => {
