    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0 rounded-t-2xl">
          <div>
            <h3 className="text-xl font-bold text-slate-900">{isEdit ? 'Edit Room Allocation' : 'New Room Allocation'}</h3>
            <p className="text-sm text-slate-500 mt-1">Assign occupants to hostel rooms and configure fees.</p>
          </div>
        ...
        <form onSubmit={submit} className="flex flex-col flex-1 overflow-hidden">
          <div className="p-6 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="sm:col-span-2">
                <label className={labelClass}>Select Occupant (User) <span className="text-rose-500">*</span></label>
              ...
              <div className="sm:col-span-2">
                <label className={labelClass}>Assign Hostel &amp; Room <span className="text-rose-500">*</span></label>
                <select value={data.hostel_room_id} onChange={handleRoomChange} required className={`${inputClass} bg-white`}>
                ...
                {errors.hostel_room_id && <p className="text-rose-500 text-xs mt-1">{errors.hostel_room_id}</p>}
              </div>

              <div>
                <label className={labelClass}>Allocation Date <span className="text-rose-500">*</span></label>
                <input
                  type="date"
                ...
                />
                {errors.allocation_date && <p className="text-rose-500 text-xs mt-1">{errors.allocation_date}</p>}
              </div>

              <div>
                <label className={labelClass}>Monthly Fee (৳) <span className="text-rose-500">*</span></label>
                <input
                  type="number"
                ...
                />
                {errors.monthly_fee && <p className="text-rose-500 text-xs mt-1">{errors.monthly_fee}</p>}
              </div>
              ...
              <div className="sm:col-span-2 pt-2 border-t border-slate-100">
                <label className="flex items-center gap-3 cursor-pointer group w-max">
                  ...
                  <span className="text-sm font-semibold text-slate-700 group-hover:text-slate-900 transition-colors">Allocation is Active</span>
                </label>
              </div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex flex-col-reverse sm:flex-row items-center justify-end gap-3 shrink-0 rounded-b-2xl">
            ...
            <button type="submit" disabled={processing} className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-all shadow-md shadow-indigo-500/20 disabled:opacity-70 active:scale-95">
              ...
              {processing ? 'Saving...' : (isEdit ? 'Update Allocation' : 'Save Allocation')}
            </button>
          </div>
        </form>
      </div>
