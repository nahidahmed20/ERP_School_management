...
export default function Index({ accounts, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.accounting.chart.index'), { 
  ...
  const getTypeBadge = (type) => {
    switch (type) {
      case 'Asset': return 'bg-sky-50 text-sky-700 border-sky-200'; // Banks, Cash
      case 'Liability': return 'bg-rose-50 text-rose-700 border-rose-200'; // Loans, Payables
      case 'Income': return 'bg-emerald-50 text-emerald-700 border-emerald-200'; // Fees, Donations
      case 'Expense': return 'bg-amber-50 text-amber-700 border-amber-200'; // Salaries, Bills
      case 'Equity': return 'bg-purple-50 text-purple-700 border-purple-200'; // Capital
  ...
  const exportToCSV = () => {
    if (!accounts.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Account Code', 'Account Name', 'Category', 'Opening Balance', 'Status'];
    const rows = accounts.data.map(item => [
      item.code || 'N/A', 
      item.name || 'N/A', 
      item.type || 'N/A', 
  ...
  const copyToClipboard = () => {
    ...
    let text = "Code\tAccount Name\tCategory\tOpening Balance\tStatus\n";
    accounts.data.forEach(item => {
      text += `${item.code || '-'}\t${item.name}\t${item.type}\t${item.opening_balance}\t${item.is_active ? 'Active' : 'Disabled'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Chart of Accounts &amp; Banks - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Chart of Accounts</h1>
            <p className="text-sm text-slate-500 mt-1">অ্যাকাউন্টিং লেজার, ব্যাংক এবং বিভিন্ন খাতের হিসাব পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add New Account
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider w-16 text-center">SL</th>
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Category (Type)</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Opening Balance</th>
                ...
                {accounts.data.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">No accounts found in the chart.</p>
                      <p className="text-xs text-slate-400 mt-1">Try a different search, or create a new account</p>
                    </td>
                  </tr>
                ) : (
                  accounts.data.map((item, index) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4 text-sm font-semibold text-slate-500 text-center">
                        {(accounts.from ?? 1) + index}
                      </td>
                      <td className="px-6 py-4">
                        <span className="inline-flex px-2 py-1 bg-slate-100 text-slate-700 border border-slate-200 rounded text-xs font-mono font-bold tracking-widest">
                          {item.code || 'N/A'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        ...
                        {item.description && (
                          <div className="text-xs text-slate-500 font-medium mt-0.5 max-w-[200px] truncate" title={item.description}>
                            {item.description}
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase border ${getTypeBadge(item.type)}`}>
                          {item.type}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase ${item.is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}>
                          {item.is_active ? 'Active' : 'Disabled'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setIsFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Account">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Account">
      ...
      {deletingItem && (
        <ConfirmDeleteModal
          ...
          onConfirm={() => {
            router.delete(route('admin.accounting.chart.destroy', deletingItem.id), {
