...
export default function Index({ enrolledUsers, campuses, activeCampusId, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.biometric-enrolledusers.index'), {
  ...
  const exportToCSV = () => {
    ...
    const rows = enrolledUsers.data.map(item => [
      ...
      item.user_id || 'N/A',
      item.user_type || 'N/A',
      item.rfid_card_no || 'N/A',
      item.is_active ? 'Active' : 'Inactive'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Enrolled_Users_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!enrolledUsers.data.length) return;
    let text = "Machine ID\tUser Name\tUser ID\tType\tRFID\tStatus\n";
    enrolledUsers.data.forEach(item => {
      text += `${item.biometric_id}\t${item.user_name}\t${item.user_id}\t${item.user_type}\t${item.rfid_card_no || '-'}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Enrolled Users Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Enroll New User
          </button>
        </div>
        ...
        <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            ...
            <div className="relative flex-1 min-w-[200px] sm:w-80">
              ...
              <input
                type="text"
                placeholder="Search Name, Machine ID..."
                value={search}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Machine ID (Biometric)</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">User Details</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">User Type</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">RFID Card No.</th>
                  ...
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-3 border border-slate-100">
                        <Icon name="fingerprint" className="w-8 h-8 text-slate-300" />
                      </div>
                      <p className="text-sm font-semibold text-slate-600">No users enrolled yet.</p>
                      <p className="text-xs text-slate-400 mt-1">Try enrolling a new user to the machine</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <strong className="text-sm font-black text-indigo-600 font-mono tracking-wide">{item.biometric_id}</strong>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <strong className="text-sm font-bold text-slate-900 block">{item.user_name}</strong>
                        <span className="text-xs font-mono text-slate-500 block mt-0.5">System ID: {item.user_id}</span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        {item.rfid_card_no ? (
                          <span className="text-sm font-mono font-medium text-slate-700">{item.rfid_card_no}</span>
                        ) : (
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setIsFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Enrollment">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Enrollment">
      ...
      {deletingItem && (
        <ConfirmDeleteModal
          item={{ name: deletingItem.user_name }}
