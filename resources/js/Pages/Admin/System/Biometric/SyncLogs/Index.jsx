...
export default function Index({ logs, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.biometric.synclogs'), {
      search, sync_status: statusFilter, per_page: perPage, ...overrides
    }, { preserveState: true, replace: true });
  }

  useEffect(() => {
    if (perPage !== (filters.per_page ?? '50') || statusFilter !== (filters.sync_status ?? '')) {
      applyFilters({ per_page: perPage, sync_status: statusFilter });
    }
  ...
  const exportToCSV = () => {
    ...
    const rows = logs.data.map(item => [
      ...
      item.enrolled_user ? `${item.enrolled_user.user_name} (${item.biometric_id})` : `Unknown (${item.biometric_id})`,
      item.device?.name || 'Deleted Device',
      item.punch_state || 'N/A',
      item.sync_status || 'N/A'
    ]);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Punch Time\tUser\tDevice\tState\tSync Status\n";
    logs.data.forEach(item => {
      let user = item.enrolled_user ? item.enrolled_user.user_name : 'Unknown';
      text += `${new Date(item.punch_time).toLocaleString()}\t${user}\t${item.device?.name || 'Unknown'}\t${item.punch_state}\t${item.sync_status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Device Sync Logs - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            ...
            <select
              value={statusFilter}
              onChange={(e) => { setStatusFilter(e.target.value); applyFilters({ sync_status: e.target.value }); }}
            ...
            <div className="relative flex-1 min-w-[200px] sm:w-64">
              ...
              <input
                type="text"
                placeholder="Search User or Bio ID..."
                value={search}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Punch Time</th>
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">State</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Sync Status</th>
                ...
                {logs.data.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">No sync logs found.</p>
                      <p className="text-xs text-slate-400 mt-1">Logs will appear when a device pushes data.</p>
                    </td>
                  </tr>
                ) : (
                  logs.data.map((item, index) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <strong className="text-sm font-black text-indigo-700 font-mono block">
                          {new Date(item.punch_time).toLocaleTimeString()}
                        </strong>
                        <span className="text-xs font-semibold text-slate-500 font-mono block mt-0.5">
                          {new Date(item.punch_time).toLocaleDateString()}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        {item.enrolled_user ? (
                          <>
                            <strong className="text-sm font-bold text-slate-900 block">{item.enrolled_user.user_name}</strong>
                            <div className="flex items-center gap-1.5 mt-0.5">
                              <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-100">{item.enrolled_user.user_type}</span>
                              <span className="text-xs font-mono text-slate-500">Bio ID: {item.biometric_id}</span>
                            </div>
                          </>
                        ) : (
                          <div className="flex flex-col">
                            <span className="text-sm font-bold text-rose-500 flex items-center gap-1">
                              <Icon name="alert-circle" className="w-3.5 h-3.5" /> Unknown User
                            </span>
                            <span className="text-xs font-mono text-slate-500 mt-0.5">Bio ID: {item.biometric_id}</span>
                          </div>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-semibold text-slate-700">
                          {item.device?.name || <span className="text-slate-400 italic">Deleted Device</span>}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase border ${
                          item.sync_status === 'Success' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'
                        }`}>
                          {item.sync_status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right no-print">
                        <button
                          className="px-3 py-1.5 text-xs font-bold bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-indigo-600 rounded-lg shadow-sm transition-all"
