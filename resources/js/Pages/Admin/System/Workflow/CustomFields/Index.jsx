...
export default function Index({ customFields, campuses, activeCampusId, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.workflow-customfields.index'), {
  ...
  const exportToCSV = () => {
    if (!customFields.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Field Label', 'DB Name', 'Assigned To', 'Field Type', 'Required', 'Status'];
    const rows = customFields.data.map(item => [
      item.field_label || 'N/A',
      item.field_name || 'N/A',
      item.target_model || 'N/A',
      item.field_type || 'N/A',
  ...
  const copyToClipboard = () => {
    if (!customFields.data.length) return;
    let text = "Field Label\tAssigned To\tType\tRequired\tStatus\n";
    customFields.data.forEach(item => {
      text += `${item.field_label}\t${item.target_model}\t${item.field_type}\t${item.is_required ? 'Yes' : 'No'}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Custom Fields Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Custom Fields</h1>
            <p className="text-sm text-slate-500 mt-1">প্রোফাইল বা ফর্মের জন্য ডাইনামিক কাস্টম ফিল্ড তৈরি করুন।</p>
          </div>
          <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Field Label (DB Name)</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Assigned To</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Field Type</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Required</th>
                ...
                {customFields.data.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">No custom fields created yet.</p>
                      <p className="text-xs text-slate-400 mt-1">Try adding a new custom field</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <strong className="text-sm font-bold text-slate-900 block">{item.field_label}</strong>
                            <span className="text-[11px] font-mono text-slate-500 block mt-0.5">{item.field_name}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-center">
                        {item.is_required ? (
                          <span className="inline-flex items-center gap-1 text-[11px] font-bold text-rose-600 uppercase tracking-wide">
                            Yes <span className="text-rose-500">*</span>
                          </span>
                        ) : (
                          <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wide">No</span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-center">
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setIsFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Field">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Field">
      ...
      {deletingItem && (
        <ConfirmDeleteModal
          item={{ name: deletingItem.field_label }}
