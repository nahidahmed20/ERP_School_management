...
export default function Index({ approvals, campuses, activeCampusId, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.workflow-approvals.index'), {
      search, status: statusFilter, per_page: perPage, ...overrides
    }, { preserveState: true, replace: true });
  }

  useEffect(() => {
    if (perPage !== (filters.per_page ?? '10') || statusFilter !== (filters.status ?? '')) {
  ...
  const exportToCSV = () => {
    if (!approvals.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Request Title', 'Details', 'Type', 'Requester', 'Status'];
    ...
    link.setAttribute("download", `Workflow_Approvals_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Title\tType\tRequester\tStatus\n";
    approvals.data.forEach(item => {
      text += `${item.title}\t${item.type}\t${item.requester_name}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Approval Workflows Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Approval Workflows</h1>
            <p className="text-sm text-slate-500 mt-1">ছুটি, খরচ বা অন্যান্য সাধারণ রিকোয়েস্ট তৈরি করুন এবং রিভিউ বা অ্যাপ্রুভ করুন।</p>
          </div>
          <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider w-[40%]">Request Title &amp; Details</th>
                ...
                {approvals.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Try creating a new workflow request</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <strong className="text-sm font-bold text-slate-900 block leading-tight">{item.title}</strong>
                      ...
                      <td className="px-6 py-4">
                        <strong className="text-sm font-semibold text-slate-800">{item.requester_name}</strong>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase border ${getStatusStyle(item.status)}`}>
                          {item.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setProcessingItem(item)} className="px-3 py-1.5 text-xs font-bold bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-indigo-600 rounded-lg shadow-sm transition-all" title="Review Request">
                            Review
                          </button>
