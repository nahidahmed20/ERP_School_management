...
export default function Index({ devices, campuses, activeCampusId, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.biometric-devices.index'), {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Device Name', 'IP & Port', 'Serial No.', 'Status', 'Last Sync'];
    ...
    link.setAttribute("download", `Biometric_Devices_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Device Name\tIP & Port\tSerial No.\tStatus\tLast Sync\n";
    devices.data.forEach(item => {
      text += `${item.name}\t${item.ip_address}:${item.port}\t${item.serial_number}\t${item.status}\t${item.last_sync ? new Date(item.last_sync).toLocaleString() : 'Never synced'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Biometric Devices Registry - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">সিস্টেমে সংযুক্ত বায়োমেট্রিক ডিভাইসগুলোর তালিকা পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Device
          </button>
        </div>
        ...
        <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            ...
            <div className="relative flex-1 min-w-[200px] sm:w-80">
              ...
              <input
                type="text"
                placeholder="Search device name, IP, Serial..."
                value={search}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">IP &amp; Port</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Serial No.</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Status</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Last Sync</th>
                ...
                {devices.data.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-3 border border-slate-100">
                        <Icon name="hard-drive" className="w-8 h-8 text-slate-300" />
                      </div>
                      <p className="text-sm font-semibold text-slate-600">No devices registered.</p>
                      <p className="text-xs text-slate-400 mt-1">Try adding a new biometric device.</p>
                    </td>
                  </tr>
                ) : (
                  devices.data.map((item, index) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded w-max border border-slate-200">
                          ...
                          <span className="text-xs text-slate-500 font-mono">:{item.port}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-semibold text-slate-700 font-mono">{item.serial_number}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase border ${
                          item.status === 'Online' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                      ...
                      <td className="px-6 py-4 text-xs font-medium text-slate-600">
                        {item.last_sync ? new Date(item.last_sync).toLocaleString() : <span className="italic text-slate-400">Never synced</span>}
                      </td>
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Device">
