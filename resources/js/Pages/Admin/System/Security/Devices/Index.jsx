...
export default function Index({ trustedDevices, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.security-devices.index'), {
  ...
  const exportToCSV = () => {
    ...
    const rows = trustedDevices.data.map(item => [
      ...
      item.last_ip_address || 'N/A',
      item.last_used_at ? new Date(item.last_used_at).toLocaleString() : 'Never'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Trusted_Devices_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!trustedDevices.data.length) return;
    let text = "User Name\tDevice Name\tLast IP Address\tLast Used\n";
    trustedDevices.data.forEach(item => {
      text += `${item.user?.name || 'Unknown User'}\t${item.device_name}\t${item.last_ip_address || '-'}\t${item.last_used_at ? new Date(item.last_used_at).toLocaleString() : 'Never'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">System Trusted Devices - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          >
            <Icon name="refresh" className="w-4 h-4 text-indigo-600" /> Refresh
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Device Name</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Last IP Address</th>
                ...
                {trustedDevices.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">No trusted devices found.</p>
                      <p className="text-xs text-slate-400 mt-1">Users' trusted devices will appear here after login.</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                          ...
                          <div>
                            {item.user ? (
                              <>
                                ...
                                <span className="text-xs font-medium text-slate-500 block mt-0.5">{item.user.email}</span>
                              </>
                            ) : (
                              <span className="text-sm font-bold text-rose-500 flex items-center gap-1">
                                <Icon name="alert-circle" className="w-4 h-4" /> Unknown User
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-lg bg-slate-50 flex items-center justify-center border border-slate-200 shrink-0 shadow-sm">
                            <Icon name="monitor" className="w-4 h-4 text-indigo-500" />
                          </div>
                          <strong className="text-sm font-bold text-slate-800 leading-tight block truncate max-w-[200px]" title={item.device_name}>
                            {item.device_name}
                          </strong>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        {item.last_ip_address ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold font-mono tracking-tight bg-slate-50 text-slate-700 border border-slate-200 shadow-sm">
                            <Icon name="globe" className="w-3.5 h-3.5 text-slate-400" /> {item.last_ip_address}
                          </span>
                        ) : (
                      ...
                      <td className="px-6 py-4">
                        {item.last_used_at ? (
                          ...
                          <span className="text-sm font-medium text-slate-400 italic">Never</span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-right no-print">
                        <button
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-rose-600 bg-rose-50 border border-rose-200 hover:bg-rose-600 hover:text-white rounded-lg shadow-sm transition-all active:scale-95"
