...
export default function Index({ templates, campuses, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.documents.certificatetemplates.index'), { 
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Template Title', 'Type', 'Status'];
    const rows = templates.data.map(item => [
      item.title || 'N/A', 
      item.template_type || 'N/A', 
      item.is_active ? 'Active' : 'Inactive'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Certificate_Templates_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Template Title\tType\tStatus\n";
    templates.data.forEach(item => {
      text += `${item.title}\t${item.template_type}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Certificate Templates Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Certificate Templates</h1>
            <p className="text-sm text-slate-500 mt-1">শিক্ষার্থীদের জন্য বিভিন্ন ধরনের সার্টিফিকেটের টেমপ্লেট ডিজাইন ও ম্যানেজ করুন।</p>
          </div>
          <Link
            href={route('admin.documents.certificatetemplates.create')}
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setViewingItem(item)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Preview Template">
                            <Icon name="eye" className="w-4 h-4" />
                          </button>
                          <Link href={route('admin.documents.certificatetemplates.edit', item.id)} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors inline-block" title="Edit Template">
