    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0 rounded-t-2xl">
          <div>
            <h3 className="text-xl font-bold text-slate-900">{isEdit ? 'Edit Certificate Template' : 'Create Certificate Template'}</h3>
            <p className="text-sm text-slate-500 mt-1">Basic configuration for certificates.</p>
          </div>
          ...
          <div className="p-6 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
            ...
            <div className="grid grid-cols-1 gap-5">
              ...
              <div>
                ...
                <input value={data.title} onChange={e => setData('title', e.target.value)} required placeholder="e.g. Annual Sports Champion" className={inputClass} />
                {errors.title && <p className="text-rose-500 text-xs mt-1">{errors.title}</p>}
              </div>

              <div>
                <label className={labelClass}>Template Type <span className="text-rose-500">*</span></label>
              ...
              <div>
                <label className={labelClass}>Certificate Body Description <span className="text-rose-500">*</span></label>
                <textarea rows="4" value={data.content_body} onChange={e => setData('content_body', e.target.value)} required placeholder="Write the main description text..." className={`${inputClass} resize-none`}></textarea>
              </div>
