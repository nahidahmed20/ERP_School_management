...
export default function Index({ templates, campuses, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.documents.transcripts.index'), { 
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Template Title', 'Grading System', 'Status'];
    const rows = templates.data.map(item => [
      item.title || 'N/A', 
      item.grading_system || 'N/A', 
      item.is_active ? 'Active' : 'Inactive'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Transcript_Templates_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Title\tGrading System\tStatus\n";
    templates.data.forEach(item => {
      text += `${item.title}\t${item.grading_system}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Transcript Templates Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Transcript Templates</h1>
            <p className="text-sm text-slate-500 mt-1">স্টুডেন্টদের মার্কশিট এবং একাডেমিক ট্রান্সক্রিপ্টের ডিজাইন পরিচালনা করুন।</p>
          </div>
          <Link
            href={route('admin.documents.transcripts.create')}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Grading System</th>
                ...
                {templates.data.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">কোনো ট্রান্সক্রিপ্ট টেমপ্লেট পাওয়া যায়নি</p>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <Link href={route('admin.documents.transcripts.edit', item.id)} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors inline-block" title="Edit Template">
