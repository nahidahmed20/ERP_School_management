...
export default function Index({ templates, campuses, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.documents.idcards.index'), { 
  ...
  const exportToCSV = () => {
    if (!templates.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Template Title', 'Layout', 'Theme Color', 'Status'];
    const rows = templates.data.map(item => [
      item.title || 'N/A', 
      item.layout_type || 'N/A', 
      item.theme_color || 'N/A',
      item.is_active ? 'Active' : 'Inactive'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `ID_Card_Templates_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Title\tLayout\tTheme Color\tStatus\n";
    templates.data.forEach(item => {
      text += `${item.title}\t${item.layout_type}\t${item.theme_color}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">ID Card Templates Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">শিক্ষার্থী এবং স্টাফদের জন্য বিভিন্ন ডিজাইনের আইডি কার্ড টেমপ্লেট পরিচালনা করুন।</p>
          </div>
          <Link
            href={route('admin.documents.idcards.create')}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Layout</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Color Theme</th>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-3 border border-slate-100">
                        <Icon name="credit-card" className="w-8 h-8 text-slate-300" />
                      </div>
                      <p className="text-sm font-semibold text-slate-600">কোনো টেমপ্লেট পাওয়া যায়নি</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or create a new template</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-md shadow-sm border border-slate-200" style={{ background: item.theme_color }}></div>
                          <span className="text-xs font-mono text-slate-500">{item.theme_color}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setViewingItem(item)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Live Preview">
                            <Icon name="eye" className="w-4 h-4" />
                          </button>
                          <Link href={route('admin.documents.idcards.edit', item.id)} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors inline-block" title="Edit Template">
                            <Icon name="edit" className="w-4 h-4" />
                          </Link>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Template">
