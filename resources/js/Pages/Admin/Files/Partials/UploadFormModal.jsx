        ...
        <form onSubmit={submit} className="flex flex-col flex-1 overflow-hidden">
          <div className="p-6 bg-white space-y-5 overflow-y-auto custom-scrollbar">
            ...
            <div>
              ...
              {errors.folder_id && <p className="text-rose-500 text-xs font-medium mt-1.5">{errors.folder_id}</p>}
            </div>

            {/* File Input */}
            <div>
              <label className={labelClass}>Select File <span className="text-rose-500">*</span></label>
              <div className="relative">
                <input
                  type="file"
                  onChange={(e) => setData('file', e.target.files[0])}
                  className="block w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 transition-all border border-slate-200 rounded-xl bg-slate-50 cursor-pointer focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              </div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3 shrink-0 rounded-b-2xl">
            ...
            >
              {processing ? (
                <><Icon name="loader" className="w-4 h-4 animate-spin" /> Uploading...</>
              ) : (
