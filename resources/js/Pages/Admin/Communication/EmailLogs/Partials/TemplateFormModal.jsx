    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0 rounded-t-2xl">
          <div>
            <h3 className="text-xl font-bold text-slate-900">{isEdit ? 'Edit Template' : 'Create Email Template'}</h3>
            <p className="text-sm text-slate-500 mt-1">Configure email subject, body content, and tags.</p>
          </div>
          ...
          <div className="p-6 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
            ...
            <div className="grid grid-cols-1 gap-5">
              ...
              <div>
                ...
                <input type="text" value={data.name} onChange={e => setData('name', e.target.value)} required placeholder="e.g. Fee Reminder Template" className={inputClass} autoFocus />
                {errors.name && <p className="text-rose-500 text-xs mt-1">{errors.name}</p>}
              </div>

              <div>
                <label className={labelClass}>Email Subject Line <span className="text-rose-500">*</span></label>
                <input type="text" value={data.subject} onChange={e => setData('subject', e.target.value)} required placeholder="e.g. Urgent: Fee Due for {student_name}" className={inputClass} />
                {errors.subject && <p className="text-rose-500 text-xs mt-1">{errors.subject}</p>}
              </div>

              <div>
                <label className={labelClass}>Available Variables (Comma separated)</label>
                <input type="text" value={data.variables} onChange={e => setData('variables', e.target.value)} placeholder="{name}, {date}" className={inputClass} />
                <span className="text-xs text-slate-400 mt-1 block">Define what dynamic tags can be used in this template.</span>
              </div>

              <div>
                <label className={labelClass}>Email Body (HTML allowed) <span className="text-rose-500">*</span></label>
                <textarea 
                  rows="6" 
                  value={data.body} 
