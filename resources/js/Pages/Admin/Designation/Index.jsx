...
export default function Index({ designations, campuses, filters }) {
  ...
  const exportToCSV = () => {
    if (!designations.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Designation Name', 'Description', 'Status'];
    const rows = designations.data.map(item => [
      item.name || 'N/A', 
  ...
  const copyToClipboard = () => {
    if (!designations.data.length) return;
    let text = "Designation Name\tDescription\tStatus\n";
    designations.data.forEach(item => {
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Designations Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Designations</h1>
            <p className="text-sm text-slate-500 mt-1">স্টাফ ও শিক্ষকদের জন্য পদবি (Designation) পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Designation
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Designation Name</th>
                ...
                {designations.data.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-slate-500">
                      কোনো পদবি (Designation) পাওয়া যায়নি।
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Designation">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Designation">
