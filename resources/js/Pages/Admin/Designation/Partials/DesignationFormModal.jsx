    ...
    >
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
          <div>
            <h3 className="text-xl font-bold text-slate-900">{isEdit ? 'Edit Designation' : 'Add New Designation'}</h3>
            <p className="text-sm text-slate-500 mt-1">Configure staff or teacher designation details.</p>
          </div>
        ...
        <form onSubmit={submit} className="flex flex-col flex-1 overflow-hidden">
          <div className="p-6 overflow-y-auto space-y-5 flex-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="sm:col-span-2">
                ...
                <WorkingCampusField value={data.campus_id} campuses={campuses} className={`${inputClass} disabled:bg-slate-100 disabled:opacity-60 disabled:cursor-not-allowed`} />
                {errors.campus_id && <p className="text-rose-500 text-xs mt-1">{errors.campus_id}</p>}
              </div>

              <div className="sm:col-span-2">
                <label className={labelClass}>Designation Name <span className="text-rose-500">*</span></label>
                <input
                  value={data.name}
              ...
              <div className="sm:col-span-2 pt-2 border-t border-slate-100">
                <label className="flex items-center gap-3 cursor-pointer group w-max">
                  ...
                  <span className="text-sm font-semibold text-slate-700 group-hover:text-slate-900 transition-colors">Designation is Active</span>
                </label>
              </div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3 shrink-0">
            ...
            <button type="submit" disabled={processing} className="flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-all shadow-md shadow-indigo-500/20 disabled:opacity-70 active:scale-95">
              {processing ? 'Saving...' : (isEdit ? 'Update Designation' : 'Create Designation')}
            </button>
          </div>
        </form>
      </div>
