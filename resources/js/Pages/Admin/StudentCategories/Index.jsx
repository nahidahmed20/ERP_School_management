...
export default function Index({ categories, campuses, filters }) {
  ...
  const exportToCSV = () => {
    if (!categories.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    ...
    link.setAttribute("download", `Student_Categories_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Category Name\tDescription\tStatus\n";
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Student Categories - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <span className="text-xs font-bold tracking-wider text-indigo-600 uppercase">Academics / Students</span>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Student Categories</h1>
            <p className="text-sm text-slate-500 mt-1">স্টুডেন্টদের বিভিন্ন ক্যাটাগরি (যেমন: Regular, Scholarship) পরিচালনা করুন।</p>
          </div>
          <button
