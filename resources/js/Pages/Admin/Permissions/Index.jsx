...
export default function PermissionIndex({ permissions, filters }) {
  ...
  const exportToCSV = () => {
    if (!permissions.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['ID', 'Permission Name'];
    const rows = permissions.data.map(item => [
      item.id,
      item.name || 'N/A'
    ]);
  ...
  const copyToClipboard = () => {
    if (!permissions.data.length) return;
    let text = "ID\tPermission Name\n";
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">System Permissions Directory - {new Date().toLocaleDateString('en-GB')}</div>
                ...
                {permissions.data.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">কোনো পারমিশন পাওয়া যায়নি।</p>
                      <p className="text-xs text-slate-400 mt-1">Start by adding a new permission.</p>
                    </td>
                  </tr>
                ) : (
                  permissions.data.map((permission) => (
                    <tr key={permission.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="inline-flex items-center gap-2 text-sm font-bold text-indigo-700 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 shadow-sm">
