...
export default function Index({ tasks, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.saas.tasks.index'), { 
  ...
  const exportToCSV = () => {
    if (!tasks.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Task Name', 'Command', 'Frequency (Cron)', 'Last Run', 'Status'];
    const rows = tasks.data.map(item => [
      item.name || 'N/A', 
      `php artisan ${item.command}`, 
      item.frequency || 'N/A', 
      item.last_run_at ? new Date(item.last_run_at).toLocaleString() : 'Not run yet',
      item.is_active ? 'Active' : 'Paused'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Scheduled_Tasks_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Task Name\tCommand\tFrequency\tLast Run\tStatus\n";
    tasks.data.forEach(item => {
      text += `${item.name}\tphp artisan ${item.command}\t${item.frequency}\t${item.last_run_at ? new Date(item.last_run_at).toLocaleString() : 'Not run yet'}\t${item.is_active ? 'Active' : 'Paused'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Scheduled Tasks (Cron) - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Scheduled Tasks (Cron)</h1>
            <p className="text-sm text-slate-500 mt-1">সিস্টেমের অটোমেটেড ব্যাকগ্রাউন্ড টাস্ক এবং কমান্ড শিডিউলার ম্যানেজ করুন।</p>
          </div>
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            >
              <Icon name="refresh" className="w-4 h-4 text-indigo-600" /> Refresh Status
            </button>
            <button
            ...
            >
              <Icon name="plus" className="w-4 h-4" /> Add Task
            </button>
          </div>
        </div>
        ...
        <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            ...
            <div className="relative flex-1 min-w-[200px] sm:w-80">
              ...
              <input
                type="text"
                placeholder="Search Task or Command..."
                value={search}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider w-[35%]">Task Name &amp; Command</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Frequency (Cron)</th>
                ...
                {tasks.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">No scheduled tasks created yet.</p>
                      <p className="text-xs text-slate-400 mt-1">Automate your tasks by scheduling them here.</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-start gap-3">
                          <div className="w-10 h-10 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center border border-indigo-100 shrink-0 mt-1">
                            <Icon name="terminal" className="w-5 h-5" />
                          </div>
                          <div>
                            <strong className="text-sm font-bold text-slate-900 block">{item.name}</strong>
                            <div className="inline-flex items-center gap-1.5 mt-1.5 bg-slate-800 px-2 py-1 rounded border border-slate-700">
                              <span className="text-xs font-mono text-emerald-400">php artisan</span>
                              <span className="text-xs font-mono text-slate-300">{item.command}</span>
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <code className="inline-flex items-center px-2.5 py-1 text-[13px] font-bold tracking-wider font-mono bg-slate-100 text-slate-700 border border-slate-200 rounded-lg shadow-sm">
                          {item.frequency}
                        </code>
                      </td>
                      <td className="px-6 py-4">
                        {item.last_run_at ? (
                          <>
                            <strong className="text-sm font-black text-slate-700 font-mono block">
                              {new Date(item.last_run_at).toLocaleDateString('en-GB')}
                            </strong>
                            <span className="text-xs font-semibold text-slate-500 font-mono block mt-0.5">
                              {new Date(item.last_run_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </span>
                          </>
                        ) : (
                          <span className="text-xs font-medium text-slate-400 italic">Not run yet</span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-center">
                        <div className="flex flex-col items-center gap-1">
                          ...
                          {item.last_status === 'Failed' && (
                            <span className="text-[10px] font-bold text-rose-500 uppercase tracking-wider flex items-center gap-1 mt-0.5">
                              <Icon name="alert-circle" className="w-3 h-3" /> Failed
                            </span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setIsFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Task">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Task">
