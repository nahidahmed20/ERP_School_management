...
export default function Index({ backups, filters }) {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `System_Backups_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!backups.data.length) return;
    let text = "File Name\tBackup Type\tSize\tStatus\n";
    backups.data.forEach(item => {
      text += `${item.file_name}\t${item.type}\t${item.file_size || 'N/A'}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">System Backups Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <button
            ...
            className="w-full sm:w-auto inline-flex justify-center items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl text-sm font-semibold transition-all shadow-md shadow-slate-900/20 active:scale-95"
          >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider w-[35%]">File Name</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Backup Type</th>
                ...
                {backups.data.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">No backups created yet.</p>
                      <p className="text-xs text-slate-400 mt-1">Generate your first system backup securely.</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <strong className="text-sm font-bold text-slate-800 font-mono break-all leading-tight">{item.file_name}</strong>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        {item.file_size ? (
                          <span className="text-sm font-semibold text-slate-700 font-mono bg-slate-100 px-2 py-1 rounded border border-slate-200">{item.file_size}</span>
                        ) : (
                          <span className="text-xs font-medium text-slate-400 italic">Calculating...</span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        ...
                        <span className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase border ${getStatusBadge(item.status)}`}>
                          {item.status === 'Pending' ? (
                            <span className="flex items-center gap-1.5">
                              <Icon name="loader" className="w-3 h-3 animate-spin" /> {item.status}
                            </span>
                          ) : (
                            item.status
                          )}
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button 
                            className={`p-2 rounded-lg transition-colors ${item.status === 'Completed' ? 'text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700' : 'text-slate-300 cursor-not-allowed'}`} 
                            disabled={item.status !== 'Completed'} 
                            title={item.status === 'Completed' ? 'Download Backup' : 'Not ready for download'}
                          >
                            <Icon name="download" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Backup">
