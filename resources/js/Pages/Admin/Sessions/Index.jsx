...
export default function Index({ sessions, campuses, filters }) {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `Academic_Sessions_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Session Name\tCampus\tStart Date\tEnd Date\tCurrent\tStatus\n";
    sessions.data.forEach(item => {
      text += `${item.name || 'N/A'}\t${item.campus?.name || 'All Campuses'}\t${item.start_date || 'N/A'}\t${item.end_date || 'N/A'}\t${item.is_current ? 'Yes' : 'No'}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Academic Sessions</h1>
            <p className="text-sm text-slate-500 mt-1">শিক্ষাবর্ষ / সেশন তৈরি ও ব্যবস্থাপনা করুন।</p>
          </div>
          <button
            onClick={openCreate}
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Session
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Start</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">End</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-center">Current?</th>
                ...
                {sessions.data.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-12 text-center text-slate-500">
                      কোনো Academic Session পাওয়া যায়নি।
                    </td>
                  </tr>
                ) : (
                  sessions.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {item.is_current && <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-wide">Current Session</span>}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex px-2.5 py-0.5 rounded text-[11px] font-bold uppercase ${item.is_current ? 'bg-indigo-50 text-indigo-700 border border-indigo-200' : 'bg-slate-100 text-slate-600'}`}>
                          {item.is_current ? 'Yes' : 'No'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center">
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => openEdit(item)} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Session">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Session">
