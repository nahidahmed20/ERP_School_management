...
export default function Index({ outlets, campuses, activeCampusId, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.cafeteria.outlets.index'), {
  ...
  const handleStatusToggle = (item) => {
    router.put(route('admin.cafeteria.outlets.update', item.id), {
      ...
      preserveState: true,
      onSuccess: () => {
        Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Outlet status updated!', showConfirmButton: false, timer: 2000 });
      }
    });
  };
  ...
  const exportToCSV = () => {
    if (!outlets.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Outlet Name', 'Location', 'Manager Name', 'Phone', 'Status'];
    const rows = outlets.data.map(item => [
      item.name || 'N/A',
      item.location || 'N/A',
    ...
    link.setAttribute("download", `Cafeteria_Outlets_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Outlet Name\tLocation\tManager\tPhone\tStatus\n";
    outlets.data.forEach(item => {
      text += `${item.name}\t${item.location || '-'}\t${item.manager_name || '-'}\t${item.phone || '-'}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Cafeteria Outlets Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Outlets / Canteens</h1>
            <p className="text-sm text-slate-500 mt-1">ক্যাফেটেরিয়া বা ক্যান্টিনের আউটলেটগুলোর তালিকা পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Outlet
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Outlet Name</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Location</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Manager &amp; Phone</th>
                ...
                {outlets.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-3 border border-slate-100">
                        <Icon name="home" className="w-8 h-8 text-slate-300" />
                      </div>
                      <p className="text-sm font-semibold text-slate-600">No outlets found</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or add a new outlet</p>
                    </td>
                  </tr>
                ) : (
                  outlets.data.map((item, index) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-800 block">{item.manager_name || '—'}</span>
                        <span className="text-xs text-slate-500 font-mono block mt-0.5">{item.phone || 'N/A'}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <button
                          onClick={() => handleStatusToggle(item)}
                          className={`inline-flex px-3 py-1.5 rounded-lg text-[11px] font-bold tracking-wide uppercase transition-all shadow-sm active:scale-95 border ${item.is_active ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100' : 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200'}`}
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setIsModalOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Outlet">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Outlet">
