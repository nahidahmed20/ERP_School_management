...
export default function Index({ materials, classes, subjects, filters }) {
  ...
  const getFileIcon = (ext) => {
    if (['pdf'].includes(ext)) return { name: 'file-text', color: 'text-rose-600', bg: 'bg-rose-50 border-rose-200' };
    if (['doc', 'docx'].includes(ext)) return { name: 'file-text', color: 'text-blue-600', bg: 'bg-blue-50 border-blue-200' };
    if (['xls', 'xlsx'].includes(ext)) return { name: 'file-text', color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-200' };
    if (['jpg', 'jpeg', 'png'].includes(ext)) return { name: 'image', color: 'text-purple-600', bg: 'bg-purple-50 border-purple-200' };
    return { name: 'file', color: 'text-slate-600', bg: 'bg-slate-50 border-slate-200' };
  };
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `Study_Materials_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Title\tClass\tSubject\tFile Type\tUploaded Date\n";
    materials.data.forEach(item => {
      text += `${item.title || 'N/A'}\t${item.school_class?.name || 'N/A'}\t${item.subject?.name || 'General'}\t${item.file_type || 'N/A'}\t${new Date(item.created_at).toLocaleDateString()}\n`;
    });
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Study Materials & Downloads</h1>
            <p className="text-sm text-slate-500 mt-1">শিক্ষার্থীদের জন্য লেকচার নোট, সিলেবাস এবং স্টাডি ম্যাটেরিয়াল আপলোড করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="upload" className="w-4 h-4" /> Upload Material
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Title & Details</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Class & Subject</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">File Type</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Uploaded Date</th>
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <strong className="text-sm font-bold text-slate-900 block mb-0.5">{item.title}</strong>
                          <span className="text-xs text-slate-500">By: {item.uploader?.name || 'Admin'}</span>
                        </td>
                        <td className="px-6 py-4">
                          <span className="inline-block px-2.5 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded text-xs font-bold mb-1">
                            {item.school_class?.name}
                          </span>
                          {item.subject && <div className="text-xs text-slate-500 font-medium">{item.subject.name}</div>}
                        </td>
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wide border ${iconData.bg} ${iconData.color}`}>
                            <Icon name={iconData.name} className="w-3.5 h-3.5" />
                            {item.file_type}
                          </span>
                        </td>
