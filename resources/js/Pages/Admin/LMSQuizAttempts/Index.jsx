...
export default function Index({ attempts, exams, students, campuses, filters }) {
  ...
  const getStatusBadge = (status) => {
    ...
    return 'bg-amber-50 text-amber-700 border-amber-200'; // Pending Evaluation
  };
  ...
  const exportToCSV = () => {
    if (!attempts.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Student Name', 'Email', 'Exam Title', 'Attempt Date', 'Marks Obtained', 'Status'];
    const rows = attempts.data.map(item => [
      item.student?.name || 'N/A', 
      item.student?.email || 'N/A', 
      item.exam?.title || 'N/A', 
      item.attempt_date || 'N/A', 
  ...
  const copyToClipboard = () => {
    ...
    let text = "Student\tExam Title\tAttempt Date\tMarks\tStatus\n";
    attempts.data.forEach(item => {
      text += `${item.student?.name || '-'}\t${item.exam?.title || '-'}\t${item.attempt_date}\t${item.obtained_marks}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Exam Results Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Quiz Attempts / Exam Results</h1>
            <p className="text-sm text-slate-500 mt-1">স্টুডেন্টদের অনলাইন পরীক্ষার ফলাফল দেখুন এবং মূল্যায়ন করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Manual Entry
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Attempt Date</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Marks Obtained</th>
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-3 border border-slate-100">
                        <Icon name="check-circle" className="w-8 h-8 text-slate-300" />
                      </div>
                      <p className="text-sm font-semibold text-slate-600">কোনো পরীক্ষার ফলাফল পাওয়া যায়নি</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or add a manual entry</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <span className="text-sm font-bold text-slate-900 block">{item.student?.name}</span>
                            <span className="text-xs text-slate-500 font-medium block mt-0.5">{item.student?.email}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-800 block">{item.exam?.title}</span>
                        <span className="text-xs text-slate-500 font-medium block mt-0.5">Total Marks: {item.exam?.total_marks}</span>
                      </td>
                      <td className="px-6 py-4 text-sm font-semibold text-slate-600 font-mono">
                        {item.attempt_date}
                      </td>
                      <td className="px-6 py-4 text-right">
                        <span className={`text-sm font-black font-mono ${item.status === 'Passed' ? 'text-emerald-700' : (item.status === 'Failed' ? 'text-rose-600' : 'text-slate-800')}`}>
                          {item.obtained_marks}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setViewingItem(item)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="View Result">
                            <Icon name="eye" className="w-4 h-4" />
                          </button>
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Evaluate / Edit">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Result">
      ...
      {deletingItem && (
        <ConfirmDeleteModal 
          item={{ name: `${deletingItem.student?.name}'s result` }} 
