...
export default function Index({ issues, books, users, campuses, filters }) {
  ...
  const exportToCSV = () => {
    if (!issues.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Borrower', 'Email', 'Book Title', 'Issue Date', 'Due Date', 'Fine Amount', 'Status'];
    const rows = issues.data.map(item => [
      item.user?.name || 'N/A',
      item.user?.email || 'N/A',
      item.book?.title || 'N/A',
      item.issue_date || 'N/A',
    ...
    link.setAttribute("download", `Library_Issues_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Borrower\tBook Title\tIssue Date\tDue Date\tFine\tStatus\n";
    issues.data.forEach(item => {
      text += `${item.user?.name}\t${item.book?.title}\t${item.issue_date}\t${item.due_date}\t${item.fine_amount}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Book Issues Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Issue Book
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Borrower (User)</th>
                ...
                {issues.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-3 border border-slate-100">
                        <Icon name="book" className="w-8 h-8 text-slate-300" />
                      </div>
                      <p className="text-sm font-semibold text-slate-600">কোনো বই ইস্যুর রেকর্ড পাওয়া যায়নি</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or issue a new book</p>
                    </td>
                  </tr>
                ) : (
                  issues.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-900 block">{item.book?.title}</span>
                        <span className="text-xs text-slate-500 font-mono block mt-0.5">ISBN: {item.book?.isbn_no || 'N/A'}</span>
                      </td>
                      <td className="px-6 py-4 text-xs text-slate-600 space-y-0.5 font-mono">
                        <div>Issue: {item.issue_date}</div>
                        <div className="text-rose-600 font-semibold">Due: {item.due_date}</div>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <span className={`text-sm font-black font-mono ${Number(item.fine_amount) > 0 ? 'text-rose-600' : 'text-slate-700'}`}>
                          ৳ {item.fine_amount}
                        </span>
                      </td>
      ...
      {deletingItem && (
        <ConfirmDeleteModal
          item={{ name: `Issue record for ${deletingItem.book?.title}` }}
