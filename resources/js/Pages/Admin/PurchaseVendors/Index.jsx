...
export default function Index({ vendors, campuses, filters }) {
  ...
  const handleStatusToggle = (id, currentStatus) => {
    router.patch(route('admin.purchase.vendors.update-status', id), {
      is_active: !currentStatus
    }, {
      preserveScroll: true,
      onSuccess: () => {
        Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Supplier status updated!', showConfirmButton: false, timer: 2000 });
      }
    });
  };
  ...
  const handleDeleteConfirm = () => {
    if (deleteId) {
      router.delete(route('admin.purchase.vendors.destroy', deleteId), {
  ...
  const exportToCSV = () => {
    if (!vendors.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Company Name', 'Contact Person', 'Phone', 'Email', 'TIN/BIN', 'Status'];
    const rows = vendors.data.map(item => [
      item.name || 'N/A',
  ...
  const copyToClipboard = () => {
    ...
    let text = "Company Name\tContact Person\tPhone\tEmail\tStatus\n";
    vendors.data.forEach(item => {
      text += `${item.name}\t${item.contact_person || '-'}\t${item.phone}\t${item.email || '-'}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Suppliers Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Suppliers</h1>
            <p className="text-sm text-slate-500 mt-1">যাদের কাছ থেকে স্কুলের পণ্য বা উপকরণ কেনা হয় তাদের তালিকা।</p>
          </div>
          <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Vendor / Company</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Contact Person</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Phone &amp; Email</th>
                ...
                {vendors.data.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or add a new vendor</p>
                    </td>
                  </tr>
                ) : (
                  vendors.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            {item.tax_id && <span className="text-[11px] font-medium text-slate-500 font-mono block mt-0.5">TIN/BIN: {item.tax_id}</span>}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm font-bold text-slate-800 font-mono">{item.phone}</div>
                        <div className="text-[11px] text-slate-500 font-medium mt-0.5">{item.email || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <button
                          onClick={() => handleStatusToggle(item.id, item.is_active)}
                          className={`inline-flex px-3 py-1.5 rounded-lg text-[11px] font-bold tracking-wider uppercase transition-all shadow-sm active:scale-95 border ${item.is_active ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100' : 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200'}`}
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Vendor">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Vendor">
