...
export default function Index({ requests, users, campuses, filters }) {
  ...
  const handleStatusChange = (id, newStatus) => {
    router.patch(route('admin.purchase.requests.update-status', id), {
      ...
      preserveScroll: true,
      onSuccess: () => {
        Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Request status updated!', showConfirmButton: false, timer: 2000 });
      }
    });
  };
  ...
  const getStatusBadge = (status) => {
    switch(status) {
        case 'Pending': return 'bg-amber-50 text-amber-700 border-amber-200'; // Yellow
        case 'Approved': return 'bg-emerald-50 text-emerald-700 border-emerald-200'; // Green
        case 'Rejected': return 'bg-rose-50 text-rose-700 border-rose-200'; // Red
        case 'Completed': return 'bg-sky-50 text-sky-700 border-sky-200'; // Blue
  ...
  const exportToCSV = () => {
    if (!requests.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Title / Purpose', 'Requested By', 'Est. Amount', 'Expected Date', 'Status'];
    const rows = requests.data.map(item => [
      item.title || 'N/A',
  ...
  const copyToClipboard = () => {
    ...
    let text = "Title\tRequested By\tEst. Amount\tExpected Date\tStatus\n";
    requests.data.forEach(item => {
      text += `${item.title}\t${item.requester?.name || '-'}\t${item.estimated_amount}\t${item.expected_date}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Purchase Requests Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Purchase Requests</h1>
            <p className="text-sm text-slate-500 mt-1">স্টাফ বা ডিপার্টমেন্টের মালামাল ক্রয়ের রিকুয়েস্টগুলো এপ্রুভ করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> New Request
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Title / Purpose</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Requested By</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Est. Amount</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Expected Date</th>
                ...
                {requests.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or create a new request</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-semibold text-slate-800">{item.requester?.name}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Request">
      ...
      {deletingItem && (
        <ConfirmDeleteModal
          item={{ name: `Request: ${deletingItem.title}` }}
