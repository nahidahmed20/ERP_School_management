    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="bg-slate-50 px-6 py-5 border-b border-slate-100 flex items-center justify-between shrink-0 rounded-t-2xl">
          <div>
            <h3 className="text-xl font-bold text-slate-900">Purchase Request Details</h3>
            <p className="text-sm text-slate-500 mt-0.5">Overview of the purchase requisition.</p>
          </div>
        ...
        <div className="p-6 space-y-5 overflow-y-auto flex-1 custom-scrollbar">
          ...
          <div className="flex items-center gap-5 border-b border-slate-100 pb-5">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm font-medium text-slate-500 mt-1 flex items-center gap-1.5">
                <Icon name="user" className="w-3.5 h-3.5" /> Requested by: <span className="font-semibold text-slate-700">{item.requester?.name}</span>
              </div>
            </div>
            ...
            <div className="text-right">
              <span className={`inline-flex px-3 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase border mt-1 ${
                ...
                item.status === 'Completed' ? 'bg-sky-50 text-sky-700 border-sky-200' :
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
              <span className="block font-bold text-emerald-600/70 uppercase text-xs mb-1">Estimated Amount</span>
              <span className="font-black text-emerald-700 font-mono text-lg">৳ {item.estimated_amount}</span>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              <span className="block font-bold text-slate-400 uppercase text-xs mb-1">Expected Delivery</span>
              <span className="font-semibold text-slate-800 font-mono">{item.expected_date}</span>
            </div>
          </div>

          <div>
            <span className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 px-1">Item Description / Details</span>
          ...
          {item.admin_remark && (
            <div className="bg-rose-50 p-4 rounded-xl border border-rose-100">
              <span className="block font-bold text-rose-700 uppercase text-xs mb-1">Admin Remark</span>
              <p className="text-sm text-rose-800 whitespace-pre-wrap">{item.admin_remark}</p>
            </div>
          )}
