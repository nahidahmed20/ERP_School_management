...
export default function Index({ courses, classes, subjects, teachers, campuses, filters }) {
  ...
  // Many-to-Many রিলেশনশিপ অনুযায়ী ফিল্টার ড্রপডাউনের ডেটা সেট করা হচ্ছে
  ...
  const exportToCSV = () => {
    if (!courses.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Course Title', 'Class', 'Subject', 'Instructor', 'Status'];
    const rows = courses.data.map(item => [
      item.title || 'N/A',
    ...
    link.setAttribute("download", `LMS_Courses_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Course Title\tClass\tSubject\tInstructor\tStatus\n";
    courses.data.forEach(item => {
      text += `${item.title}\t${item.school_class?.name || '-'}\t${item.subject?.name || '-'}\t${item.teacher?.name || 'Not Assigned'}\t${item.is_published ? 'Published' : 'Draft'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">LMS Courses Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Courses</h1>
            <p className="text-sm text-slate-500 mt-1">স্টুডেন্টদের জন্য বিষয়ভিত্তিক কোর্স তৈরি ও পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Create Course
          </button>
        </div>
                ...
                {courses.data.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">কোনো কোর্স পাওয়া যায়নি</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or create a new course</p>
                    </td>
                  </tr>
                ) : (
                  courses.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {item.teacher ? (
                          <span className="text-sm font-semibold text-slate-700">{item.teacher.name}</span>
                        ) : (
                          <span className="text-xs text-slate-400 italic">Not Assigned</span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase border ${item.is_published ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                          {item.is_published ? 'Published' : 'Draft'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setViewingItem(item)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="View Course">
                            <Icon name="eye" className="w-4 h-4" />
                          </button>
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Course">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Course">
      ...
      {deletingItem && (
        <ConfirmDeleteModal
          item={{ name: `Course: ${deletingItem.title}` }}
