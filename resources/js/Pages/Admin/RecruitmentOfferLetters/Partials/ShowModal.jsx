    ...
    <div className="mm-modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <div className="bg-slate-50 px-6 py-5 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div>
            <h3 className="text-xl font-bold text-slate-900">Offer Letter Details</h3>
            <p className="text-sm text-slate-500 mt-0.5">Summary of the employment offer.</p>
          </div>
        ...
        <div className="p-6 space-y-5 overflow-y-auto max-h-[75vh]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
            <div className="sm:col-span-2 border-b border-slate-200 pb-3 mb-1">
              ...
              <span className="text-xs font-semibold text-indigo-600 mt-0.5 block">Position: {item.applicant?.job_post?.title}</span>
            </div>
            
            <div>
              <span className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Salary Offered</span>
              <strong className="text-sm font-mono text-emerald-700 mt-0.5 block">{item.salary_offered}</strong>
            </div>
            <div>
              <span className="block text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Candidate Status</span>
              <span className={`inline-flex px-3 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase border mt-1 ${
                ...
                item.status === 'Declined' ? 'bg-rose-50 text-rose-700 border-rose-200' :
          ...
          <div className="grid grid-cols-2 gap-4 text-sm px-1">
            ...
            <div>
              <span className="block font-bold text-slate-400 uppercase text-xs mb-1">Joining Date</span>
              <span className="font-bold text-indigo-700 font-mono">{item.joining_date}</span>
            </div>
          </div>

          <div className="px-1 border-t border-slate-100 pt-4">
            <span className="block font-bold text-slate-400 uppercase text-xs mb-1">Valid Until</span>
            <span className="font-mono font-semibold text-rose-600">{item.valid_until}</span>
          </div>

          <div>
            <span className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 px-1">Terms &amp; Conditions / Note</span>
            <div className="text-sm text-slate-700 bg-slate-50/50 border border-slate-200 p-4 rounded-xl min-h-[80px] whitespace-pre-wrap leading-relaxed">
              {item.terms_conditions || <span className="text-slate-400 italic">No terms provided.</span>}
            </div>
          </div>
