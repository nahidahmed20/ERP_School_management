...
export default function Index({ offerLetters, applicants, filters }) {
  ...
  useEffect(() => {
    ...
    if (errors?.applicant_id) {
       Swal.fire({ toast: true, position: 'top-end', icon: 'error', title: errors.applicant_id, showConfirmButton: false, timer: 4000, timerProgressBar: true });
    }
  }, [flash, errors]);

  const applyFilters = (overrides = {}) => {
    router.get(route('admin.recruitment.offer-letters.index'), { search, status: statusFilter, per_page: perPage, ...overrides }, { preserveState: true, replace: true });
  };
  ...
  const exportToCSV = () => {
    if (!offerLetters.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Applicant Name', 'Job Post', 'Salary Offered', 'Issue Date', 'Joining Date', 'Status'];
    const rows = offerLetters.data.map(item => [
      item.applicant?.name || 'N/A', 
      item.applicant?.job_post?.title || 'N/A', 
      item.salary_offered || 'N/A', 
      item.issue_date || 'N/A', 
    ...
    link.setAttribute("download", `Offer_Letters_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!offerLetters.data.length) return;
    let text = "Applicant\tJob Post\tSalary Offered\tIssue Date\tJoining Date\tStatus\n";
    offerLetters.data.forEach(item => {
      text += `${item.applicant?.name}\t${item.applicant?.job_post?.title || '-'}\t${item.salary_offered}\t${item.issue_date}\t${item.joining_date}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Offer Letters Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Offer Letters</h1>
            <p className="text-sm text-slate-500 mt-1">নির্বাচিত প্রার্থীদের অফার লেটার এবং জয়েনিং স্ট্যাটাস ট্র্যাক করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Create Offer Letter
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Salary Offered</th>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-3 border border-slate-100">
                        <Icon name="briefcase" className="w-8 h-8 text-slate-300" />
                      </div>
                      <p className="text-sm font-semibold text-slate-600">No offer letters found</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or issue a new offer</p>
                    </td>
                  </tr>
                ) : (
                  offerLetters.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-900 block">{item.applicant?.name}</span>
                        <div className="text-xs text-slate-500 font-medium mt-0.5 max-w-[200px] truncate" title={item.applicant?.job_post?.title}>
                          {item.applicant?.job_post?.title}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-black text-emerald-600 font-mono">{item.salary_offered}</span>
                      </td>
                      <td className="px-6 py-4 text-sm">
                        <span className="font-semibold text-slate-800 block text-xs">Issue: {item.issue_date}</span>
                        <span className="text-xs text-indigo-600 font-bold block mt-0.5">Join: {item.joining_date}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Offer Letter">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeleteId(item.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Offer Letter">
