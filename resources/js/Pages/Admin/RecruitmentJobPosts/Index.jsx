...
export default function Index({ jobPosts, filters }) {
  ...
  const applyFilters = (overrides = {}) => {
    router.get(route('admin.recruitment.job-posts.index'), { search, status: statusFilter, per_page: perPage, ...overrides }, { preserveState: true, replace: true });
  };
  ...
  const getStatusBadge = (status) => {
    if (status === 'Open') return <span className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-emerald-50 text-emerald-700 border border-emerald-200">Open</span>;
    return <span className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-rose-50 text-rose-700 border border-rose-200">Closed</span>;
  };
  ...
  const exportToCSV = () => {
    if (!jobPosts.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Job Title', 'Department', 'Type', 'Vacancies', 'Deadline', 'Status'];
    const rows = jobPosts.data.map(item => [
      item.title || 'N/A', 
      item.department || 'N/A', 
    ...
    link.setAttribute("download", `Job_Posts_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!jobPosts.data.length) return;
    let text = "Title\tDepartment\tType\tVacancies\tDeadline\tStatus\n";
    jobPosts.data.forEach(item => {
      text += `${item.title}\t${item.department || '-'}\t${item.employment_type}\t${item.vacancies}\t${item.deadline}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Recruitment Job Posts - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Job Posts</h1>
            <p className="text-sm text-slate-500 mt-1">স্কুলের নতুন নিয়োগ এবং চাকরির বিজ্ঞপ্তিগুলো পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Create Job Post
          </button>
        </div>
        ...
        <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 flex flex-col xl:flex-row items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
            ...
            <div className="relative flex-1 min-w-[200px] sm:w-64">
              ...
              <input
                type="text"
                placeholder="Search title or dept..."
                value={search}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Type &amp; Vacancies</th>
                ...
                {jobPosts.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">No job posts found</p>
                      <p className="text-xs text-slate-400 mt-1">Try a different search, or create a new post</p>
                    </td>
                  </tr>
                ) : (
                  jobPosts.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="inline-flex px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-lg text-[10px] font-bold tracking-wider uppercase mb-1">
                          {item.employment_type}
                        </span>
                        <div className="text-xs text-slate-500 font-medium">
                          {item.vacancies} Position(s)
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm font-semibold text-amber-600 font-mono">
                        {item.deadline}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Edit Job Post">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeleteId(item.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Job Post">
