...
export default function ShowModal({ item, onClose }) {
  ...
  return (
    <div className="mm-modal-overlay flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div
        className="mm-modal w-full max-w-lg bg-white rounded-2xl shadow-xl flex flex-col"
      ...
      >
        ...
        <div className="p-6 space-y-6 overflow-y-auto max-h-[75vh]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm px-1">
            <div className="sm:col-span-2">
              <span className="block font-bold text-slate-400 uppercase text-xs mb-1">Title (To / From)</span>
              <span className="font-bold text-slate-900 text-lg break-words">{item.title}</span>
            </div>
            <div className="sm:col-span-2">
          ...
          <div>
            ...
            <div className="text-sm text-slate-700 bg-slate-50/50 border border-slate-200 p-4 rounded-xl min-h-[60px] whitespace-pre-wrap leading-relaxed break-words">
              {item.note || <span className="text-slate-400 italic">No note provided.</span>}
            </div>
          </div>

          <div className="px-1 border-t border-slate-100 pt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            {item.attachment ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center justify-center w-full sm:w-auto gap-1.5 text-xs font-bold bg-indigo-50 hover:bg-indigo-100 text-indigo-700 px-4 py-2 rounded-lg transition-colors border border-indigo-200 shadow-sm"
              >
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end rounded-b-2xl">
          <button type="button" className="px-5 py-2.5 text-sm font-semibold text-slate-600 bg-white border border-slate-200 hover:bg-slate-100 rounded-xl transition-all shadow-sm w-full sm:w-auto" onClick={onClose}>
            Close
          </button>
        </div>
      </div>
