...
export default function Index({ inquiries, filters }) {
  ...
  const applyFilters = (overrides = {}) => {
    router.get(route('admin.frontoffice.admission-inquiries.index'), { search, status: statusFilter, per_page: perPage, ...overrides }, { preserveState: true, replace: true });
  };
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'Converted': return <span className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-emerald-50 text-emerald-700 border border-emerald-200">Converted</span>;
      case 'Follow-up': return <span className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-blue-50 text-blue-700 border border-blue-200">Follow-up</span>;
      case 'Cancelled': return <span className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-rose-50 text-rose-700 border border-rose-200">Cancelled</span>;
  ...
  const exportToCSV = () => {
    if (!inquiries.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Applicant Name', 'Guardian Name', 'Phone', 'Class Interested', 'Inquiry Date', 'Status'];
    const rows = inquiries.data.map(item => [
      item.applicant_name || 'N/A', 
      item.guardian_name || 'N/A', 
      item.phone || 'N/A', 
      item.class_interested || 'N/A', 
      item.inquiry_date || 'N/A', 
      item.status || 'Pending'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Admission_Inquiries_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!inquiries.data.length) return;
    let text = "Applicant\tGuardian\tPhone\tClass\tDate\tStatus\n";
    inquiries.data.forEach(item => {
      text += `${item.applicant_name}\t${item.guardian_name}\t${item.phone}\t${item.class_interested}\t${item.inquiry_date}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Admission Inquiries - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Admission Inquiries</h1>
            <p className="text-sm text-slate-500 mt-1">সম্ভাব্য শিক্ষার্থীদের ভর্তির খোঁজখবর ও লিড পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Inquiry
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Applicant / Guardian</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Phone</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Class Interested</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Inquiry Date</th>
                ...
                {inquiries.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      কোনো খোঁজখবর বা ইনকোয়ারি পাওয়া যায়নি।
                    </td>
                  </tr>
                ) : (
                  inquiries.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-900 block">{item.applicant_name}</span>
                        <span className="text-xs text-slate-500 font-medium block mt-0.5">G: {item.guardian_name}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Inquiry">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeleteId(item.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Inquiry">
      ...
      {deleteId && (
        <ConfirmDeleteModal
            ...
            message="আপনি কি নিশ্চিত যে এই ইনকোয়ারি রেকর্ডটি চিরতরে মুছে ফেলতে চান?"
        />
      )}
    </AuthenticatedLayout>
  );
