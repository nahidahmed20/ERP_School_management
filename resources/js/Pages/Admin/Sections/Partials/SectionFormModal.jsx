...
export default function SectionFormModal({ item, campuses, activeCampusId, onClose }) {
  const isEdit = !!item;
  ...
  function submit(e) {
    ...
    if (isEdit) {
      put(route('admin.sections.update', item.id), options);
    } else {
      post(route('admin.sections.store'), options);
    }
  }
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Configure section details.</p>
          </div>
        ...
        <form onSubmit={submit} className="flex flex-col flex-1 overflow-hidden">
          <div className="p-6 overflow-y-auto space-y-5 flex-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              {/* Section Name */}
              <div className="sm:col-span-2">
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">Section Name <span className="text-rose-500">*</span></label>
                <input
                  value={data.name}
                  onChange={(e) => setData('name', e.target.value)}
                  autoFocus
                  placeholder="e.g. A, B, Rose, Morning"
              ...
              <div className="sm:col-span-2 pt-2 border-t border-slate-100">
                <label className="flex items-center gap-3 cursor-pointer group w-max">
                  ...
                  <span className="text-sm font-semibold text-slate-700 group-hover:text-slate-900 transition-colors">Section is Active</span>
                </label>
              </div>
