...
export default function Index({ classrooms, campuses, filters }) {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `Classrooms_Directory_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!classrooms.data.length) return;
    let text = "Room Number\tType\tCapacity\tStatus\n";
    classrooms.data.forEach(item => {
      text += `${item.room_number || 'N/A'}\t${item.type || 'N/A'}\t${item.capacity || 'N/A'}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Classrooms</h1>
            <p className="text-sm text-slate-500 mt-1">ক্যাম্পাসের সকল ক্লাসরুম ও ল্যাব পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Classroom
          </button>
        </div>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-bold text-slate-900">{item.room_number}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Classroom">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Classroom">
