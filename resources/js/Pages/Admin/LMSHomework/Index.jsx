...
export default function Index({ homeworks, classes, subjects, campuses, filters }) {
  ...
  const exportToCSV = () => {
    if (!homeworks.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Title / Assignment', 'Class', 'Subject', 'Given Date', 'Deadline', 'Total Marks', 'Status'];
    const rows = homeworks.data.map(item => [
      item.title || 'N/A', 
      item.school_class?.name || 'N/A', 
      item.subject?.name || 'N/A', 
      item.homework_date || 'N/A', 
      item.submission_date || 'N/A', 
    ...
    link.setAttribute("download", `Homework_Assignments_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!homeworks.data.length) return;
    let text = "Title\tClass\tSubject\tGiven Date\tDeadline\tMarks\n";
    homeworks.data.forEach(item => {
      text += `${item.title}\t${item.school_class?.name || '-'}\t${item.subject?.name || '-'}\t${item.homework_date}\t${item.submission_date}\t${item.total_marks || '-'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Homework & Assignments Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Homework &amp; Assignments</h1>
            <p className="text-sm text-slate-500 mt-1">স্টুডেন্টদের ক্লাস ও বিষয়ভিত্তিক হোমওয়ার্ক তৈরি ও পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Homework
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Title / Assignment</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Class &amp; Subject</th>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-3 border border-slate-100">
                        <Icon name="edit-3" className="w-8 h-8 text-slate-300" />
                      </div>
                      <p className="text-sm font-semibold text-slate-600">কোনো হোমওয়ার্ক পাওয়া যায়নি</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or add a new homework</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-xs text-slate-500 font-medium block mt-0.5">Marks: {item.total_marks || 'N/A'}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-800 block">{item.school_class?.name}</span>
                        <span className="text-xs text-slate-500 font-medium block mt-0.5">{item.subject?.name}</span>
                      </td>
                      <td className="px-6 py-4 text-xs font-mono space-y-0.5">
                        <div className="text-slate-600">Given: {item.homework_date}</div>
                        <div className="text-rose-600 font-bold">Deadline: {item.submission_date}</div>
                      </td>
                      <td className="px-6 py-4">
                        {item.document_path ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-emerald-50 text-emerald-700 border border-emerald-200">
                            <Icon name="paperclip" className="w-3.5 h-3.5" /> Attached
                          </span>
                        ) : (
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Homework">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Homework">
      ...
      {deletingItem && (
        <ConfirmDeleteModal 
          item={{ name: `Homework: ${deletingItem.title}` }} 
