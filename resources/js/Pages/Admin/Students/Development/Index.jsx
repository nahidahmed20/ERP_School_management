...
function Form({ item, students, close }) {
    const { data, setData, post, put, processing, errors } = useForm(
        item
            ? {
                  ...blank,
                  ...item,
                  record_date: item.record_date?.slice(0, 10),
                  follow_up_date: item.follow_up_date?.slice(0, 10) || "",
    ...
    const submit = (e) => {
        ...
        item
            ? put(route("admin.student-development-records.update", item.id), o)
            : post(route("admin.student-development-records.store"), o);
    };
        ...
        >
            ...
            >
                <div className="mb-5 flex justify-between">
                    <div>
                        <h2 className="text-xl font-bold">
                            {item ? "Edit" : "Add"} Student Record
                        </h2>
                        <p className="text-sm text-slate-500">
                            Student support, achievement and development
                        </p>
                    </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                    ...
                    <label className="text-sm font-semibold">
                        ...
                        <input
                            ...
                            placeholder="Session / activity / achievement title"
                            className="mt-1 w-full rounded-xl border-slate-200"
                        />
                        {errors.title && (
                            <small className="text-rose-600">
                ...
                <div className="mt-6 flex justify-end gap-3">
                    ...
                    <button
                        disabled={processing}
                        className="rounded-xl bg-emerald-800 px-6 py-2.5 font-semibold text-white"
                    >
                        Save Record
                    </button>
                </div>
...
export default function Index({ records, studentList, filters, summary }) {
    ...
    const filter = (key, value) =>
        router.get(
            route("admin.student-development-records.index"),
            { ...filters, [key]: value },
            ...
            <div className="space-y-6 py-8 sm:px-6 lg:px-8">
                <div className="flex flex-col justify-between gap-3 sm:flex-row items-center">
                    <div>
                        ...
                        <p className="text-sm text-slate-500">
                            Counselling, scholarship, activities, achievements
                    ...
                    <button
                        ...
                        className="rounded-xl bg-emerald-800 px-5 py-2.5 font-semibold text-white"
                    >
                        + Add Student Record
                    </button>
                </div>
                <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                    {Object.entries(types).map(([k, l]) => (
                        <button
                            key={k}
                            onClick={() =>
                                filter("type", filters.type === k ? "" : k)
                            }
                            className={`rounded-xl border p-4 text-left ${filters.type === k ? "border-emerald-700 bg-emerald-50" : "bg-white"}`}
                        >
                ...
                <div className="flex flex-col gap-3 rounded-xl border bg-white p-3 sm:flex-row">
                    <input
                        ...
                        placeholder="Search student, admission no. or title..."
                        className="flex-1 rounded-xl border-slate-200"
                    />
                    <select
                        value={filters.student_id || ""}
                        onChange={(e) => filter("student_id", e.target.value)}
                        ...
                        <tbody className="divide-y">
                                    ...
                                    <td className="px-5 py-4">
                                        ...
                                        <button
                                            onClick={() =>
                                                ...
                                                router.delete(
                                                    route(
                                                        "admin.student-development-records.destroy",
                                                        r.id,
                                                    ),
                                                )
                                            }
                            ...
                            {!records.data.length && (
                                <tr>
                                    ...
                                    >
                                        No student development records found.
                                    </td>
                                </tr>
                            )}
                        </tbody>
