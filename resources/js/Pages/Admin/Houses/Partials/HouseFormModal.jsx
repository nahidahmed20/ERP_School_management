      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50 shrink-0">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">Configure student house details.</p>
          </div>
        ...
        <form onSubmit={submit} className="flex flex-col flex-1 overflow-hidden">
          <div className="p-6 overflow-y-auto space-y-5 flex-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="sm:col-span-2">
                ...
                <WorkingCampusField value={data.campus_id} campuses={campuses} className="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed" />
                {errors.campus_id && <p className="text-rose-500 text-xs mt-1">{errors.campus_id}</p>}
              </div>

              {/* House Name */}
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">House Name <span className="text-rose-500">*</span></label>
                <input
                  value={data.name}
              ...
              {/* House Color */}
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">House Color</label>
                <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl p-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setData('color', e.target.value)}
                    placeholder="#HexCode"
                    className="flex-1 bg-transparent border-0 text-sm font-mono text-slate-700 focus:ring-0 outline-none uppercase"
                  />
                </div>
              ...
              <div className="sm:col-span-2 pt-2 border-t border-slate-100">
                <label className="flex items-center gap-3 cursor-pointer group w-max">
                  ...
                  <span className="text-sm font-semibold text-slate-700 group-hover:text-slate-900 transition-colors">House is Active</span>
                </label>
              </div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3 shrink-0">
            ...
            <button type="submit" disabled={processing} className="flex items-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl transition-all shadow-md shadow-indigo-500/20 disabled:opacity-70 disabled:cursor-not-allowed active:scale-95">
              {processing ? 'Saving...' : (isEdit ? 'Update House' : 'Create House')}
            </button>
          </div>
        </form>
      </div>
