...
export default function Index({ houses, campuses, filters }) {
  ...
  const exportToCSV = () => {
    if (!houses.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['House Name', 'Description', 'Status'];
    const rows = houses.data.map(item => [
      item.name || 'N/A', 
    ...
    link.setAttribute("download", `Houses_Directory_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!houses.data.length) return;
    let text = "House Name\tDescription\tStatus\n";
    houses.data.forEach(item => {
      text += `${item.name || 'N/A'}\t${item.description || 'N/A'}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Houses Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <span className="text-xs font-bold tracking-wider text-indigo-600 uppercase">Academic Foundation</span>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Houses</h1>
            <p className="text-sm text-slate-500 mt-1">স্কুলের স্পোর্টস বা এক্সট্রা কারিকুলার হাউজ পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add House
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">House Name</th>
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {item.color ? (
                            <span className="w-4 h-4 rounded-full shrink-0 shadow-sm border border-slate-200" style={{ backgroundColor: item.color }}></span>
                          ) : (
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit House">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete House">
