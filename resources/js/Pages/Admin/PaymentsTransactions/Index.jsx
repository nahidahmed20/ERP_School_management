...
export default function Index({ transactions, gateways, filters }) {
  ...
  const applyFilters = (overrides = {}) => {
    router.get(route('admin.payments.transactions.index'), { 
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case 'Failed': return 'bg-rose-50 text-rose-700 border-rose-200 focus:ring-rose-500';
  ...
  const exportToCSV = () => {
    if (!transactions.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Transaction ID', 'Gateway / Method', 'Amount', 'Date', 'Status'];
    const rows = transactions.data.map(item => [
      item.transaction_id || 'N/A', 
      `${item.gateway?.name || 'Manual'} (${item.payment_method || '-'})`, 
  ...
  const copyToClipboard = () => {
    ...
    let text = "Transaction ID\tGateway/Method\tAmount\tDate\tStatus\n";
    transactions.data.forEach(item => {
      text += `${item.transaction_id}\t${item.gateway?.name || 'Manual'} (${item.payment_method || '-'})\t${item.amount} ${item.currency}\t${item.transaction_date}\t${item.status}\n`;
    });
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Transactions</h1>
            <p className="text-sm text-slate-500 mt-1">সব ধরনের পেমেন্ট লেনদেন এবং তাদের স্ট্যাটাস ট্র্যাক করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Manual Txn
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Gateway / Method</th>
                ...
                {transactions.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or search term</p>
                    </td>
                  </tr>
                ) : (
                  transactions.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <strong className="text-sm font-bold text-slate-900 font-mono bg-slate-100 px-2 py-0.5 rounded block w-max">{item.transaction_id}</strong>
                        <div className="text-xs font-medium text-slate-500 mt-1">Ref: {item.reference_no || 'N/A'}</div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-800 block">{item.gateway?.name || 'Manual Entry'}</span>
                        <span className="text-xs text-slate-500 font-medium block mt-0.5">{item.payment_method || '-'}</span>
                      </td>
                      <td className="px-6 py-4">
