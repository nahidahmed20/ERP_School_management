...
export default function Index({ refunds, transactions, filters }) {
  ...
  const applyFilters = (overrides = {}) => {
    router.get(route('admin.payments.refunds.index'), { 
  ...
  const handleStatusChange = (id, newStatus) => {
    router.patch(route('admin.payments.refunds.update-status', id), {
      ...
      preserveScroll: true,
      onSuccess: () => {
        Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Refund status updated!', showConfirmButton: false, timer: 2000 });
      }
    });
  };
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'Refunded': return 'bg-sky-50 text-sky-700 border-sky-200 focus:ring-sky-500';
      case 'Approved': return 'bg-emerald-50 text-emerald-700 border-emerald-200 focus:ring-emerald-500';
      case 'Rejected': return 'bg-rose-50 text-rose-700 border-rose-200 focus:ring-rose-500';
  ...
  const exportToCSV = () => {
    if (!refunds.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Original TrxID', 'Refund Amount', 'Reason', 'Date', 'Status'];
    const rows = refunds.data.map(item => [
      item.transaction?.transaction_id || 'N/A', 
      `${item.amount} ${item.transaction?.currency || 'BDT'}`, 
      item.reason || 'N/A', 
      item.refund_date || 'N/A', 
      item.status || 'Pending'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Refunds_List_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!refunds.data.length) return;
    let text = "Original TrxID\tRefund Amount\tReason\tDate\tStatus\n";
    refunds.data.forEach(item => {
      text += `${item.transaction?.transaction_id || '-'}\t${item.amount} ${item.transaction?.currency || 'BDT'}\t${item.reason}\t${item.refund_date || '-'}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Refunds Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Refunds</h1>
            <p className="text-sm text-slate-500 mt-1">ভুল পেমেন্ট বা অতিরিক্ত ফি ফেরতের রিকোয়েস্ট ম্যানেজ করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Initiate Refund
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Original TrxID</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Refund Amount</th>
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-3 border border-slate-100">
                        <Icon name="refresh" className="w-8 h-8 text-slate-300" />
                      </div>
                      <p className="text-sm font-semibold text-slate-600">No refunds found</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or initiate a new refund</p>
                    </td>
                  </tr>
                ) : (
                  refunds.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <strong className="text-sm font-bold text-slate-900 font-mono bg-slate-100 px-2 py-0.5 rounded block w-max">
                          {item.transaction?.transaction_id || 'N/A'}
                        </strong>
                        <div className="text-xs font-medium text-slate-500 mt-1">Original: {item.transaction?.amount} {item.transaction?.currency}</div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <div className="text-sm text-slate-600 max-w-[200px] truncate" title={item.reason}>
                          {item.reason}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Refund">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeleteId(item.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Refund">
