...
export default function Index({ assignments, assets, filters }) {
  ...
  const applyFilters = (overrides = {}) => {
    router.get(route('admin.purchase.asset-assignments.index'), { 
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'Returned': return 'bg-emerald-50 text-emerald-700 border-emerald-200 focus:ring-emerald-500';
      case 'Damaged': return 'bg-amber-50 text-amber-700 border-amber-200 focus:ring-amber-500';
      case 'Lost': return 'bg-rose-50 text-rose-700 border-rose-200 focus:ring-rose-500';
      default: return 'bg-indigo-50 text-indigo-700 border-indigo-200 focus:ring-indigo-500'; // Assigned
    }
  };
  ...
  const exportToCSV = () => {
    if (!assignments.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Asset Details', 'Assigned To', 'Assigned Date', 'Due/Returned', 'Status'];
    const rows = assignments.data.map(item => [
      item.asset?.name || 'Unknown Asset', 
      item.assignee_name || 'N/A', 
      item.assigned_date || 'N/A', 
      item.status === 'Returned' ? `Returned: ${item.returned_date}` : `Due: ${item.due_date || 'N/A'}`, 
      item.status || 'Assigned'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Asset_Assignments_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Asset Details\tAssigned To\tAssigned Date\tDue/Returned\tStatus\n";
    assignments.data.forEach(item => {
      let dueReturn = item.status === 'Returned' ? `Returned: ${item.returned_date}` : `Due: ${item.due_date || 'N/A'}`;
      text += `${item.asset?.name || '-'}\t${item.assignee_name}\t${item.assigned_date}\t${dueReturn}\t${item.status}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Asset Assignments Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Asset Assignments</h1>
            <p className="text-sm text-slate-500 mt-1">স্টাফদের মাঝে বিতরণ করা স্কুলের সম্পদের হিসাব রাখুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Assign Asset
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Due / Returned</th>
                ...
                {assignments.data.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or create a new assignment</p>
                    </td>
                  </tr>
                ) : (
                  assignments.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <strong className="text-sm font-bold text-slate-900 block">{item.asset?.name || 'Unknown Asset'}</strong>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-semibold text-slate-800">{item.assignee_name}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        {item.status === 'Returned' ? (
                          <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md">
                            <Icon name="check" className="w-3.5 h-3.5" /> Returned: {item.returned_date}
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-500 bg-slate-100 px-2 py-1 rounded-md">
                            <Icon name="clock" className="w-3.5 h-3.5" /> Due: {item.due_date || 'N/A'}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Assignment">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeleteId(item.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Assignment">
