...
export default function Index({ events, filters }) {
  ...
  const applyFilters = (overrides = {}) => {
    router.get(route('admin.alumni.events.index'), { search, status: statusFilter, per_page: perPage, ...overrides }, { preserveState: true, replace: true });
  };

  const handleDeleteConfirm = () => {
    if (deleteId) {
      router.delete(route('admin.alumni.events.destroy', deleteId), {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'Completed': return <span className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-emerald-50 text-emerald-700 border border-emerald-200">Completed</span>;
      ...
      default: return <span className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-blue-50 text-blue-700 border border-blue-200">Upcoming</span>;
    }
  };
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `Alumni_Events_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Event Title\tDate\tTime\tLocation\tStatus\n";
    events.data.forEach(item => {
      text += `${item.title}\t${item.date}\t${item.time}\t${item.location || '-'}\t${item.status}\n`;
    });
    ...
    <AuthenticatedLayout>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Alumni Events</h1>
            <p className="text-sm text-slate-500 mt-1">প্রাক্তন শিক্ষার্থীদের পুনর্মিলনী এবং অন্যান্য ইভেন্ট পরিচালনা করুন।</p>
          </div>
          <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Event Title</th>
                ...
                {events.data.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or create a new event</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          {item.cover_photo ? (
                            <img src={`/storage/${item.cover_photo}`} alt={item.title} className="w-10 h-10 rounded-lg object-cover border border-slate-200 shrink-0" />
                          ) : (
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-indigo-700 block">{item.date}</span>
                        <span className="text-xs font-mono text-slate-500 block mt-0.5">{item.time}</span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button onClick={() => setDeleteId(item.id)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Event">
      ...
      {deleteId && (
        <ConfirmDeleteModal
          ...
          title="Delete Event"
          message="আপনি কি নিশ্চিত যে এই ইভেন্টটি মুছে ফেলতে চান?"
        />
      )}
    </AuthenticatedLayout>
  );
