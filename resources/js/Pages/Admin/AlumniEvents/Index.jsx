...
export default function Index({ events, filters }) {
  ...
  const applyFilters = () => {
    router.get(route('admin.alumni.events.index'), { search, status: statusFilter }, { preserveState: true });
  };
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      default: return { bg: '#dbeafe', text: '#1d4ed8' }; // Upcoming
    }
  };
      ...
      header={
        <div className="page-head">
          <div>
            ...
            <h1>Alumni Events</h1>
            <p className="desc">প্রাক্তন শিক্ষার্থীদের পুনর্মিলনী এবং অন্যান্য ইভেন্ট পরিচালনা করুন।</p>
          </div>
      ...
      <div className="card mm-card">
        <div className="mm-filters" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <div className="search">
            <Icon name="search" />
            <input placeholder="Search event title or location..." value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && applyFilters()} />
          </div>
          ...
          <table className="mm-table">
            ...
            <tbody>
              {events.data.length === 0 && (
                <tr><td colSpan={5} className="mm-empty">কোনো ইভেন্ট পাওয়া যায়নি।</td></tr>
              )}
