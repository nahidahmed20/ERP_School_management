...
export default function Index({ appraisals, staffList, filters }) {
  ...
  function applyFilters(overrides = {}) {
    router.get(route('admin.staff-appraisals.index'), { search, per_page: perPage, ...overrides }, { preserveState: true, replace: true });
  }
  ...
  const renderStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      stars.push(
        <span key={i} className={`text-base ${i <= Math.round(rating) ? 'text-amber-400' : 'text-slate-200'}`}>
          ★
        </span>
      );
    }
    return (
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs font-bold text-slate-600 font-mono">({rating}/5)</span>
      </div>
    );
  };
  ...
  const exportToCSV = () => {
    if (!appraisals.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Staff Name', 'Period', 'Rating', 'Evaluator', 'Date'];
    const rows = appraisals.data.map(item => [
      `${item.staff?.first_name || ''} ${item.staff?.last_name || ''}`, 
      item.period || 'N/A', 
      item.rating || '0', 
      item.evaluator?.name || 'N/A', 
      item.appraisal_date || 'N/A'
    ]);
    const csvContent = "data:text/csv;charset=utf-8," + [headers.join(','), ...rows.map(e => e.map(val => `"${val}"`).join(','))].join('\n');
    const link = document.createElement("a");
    link.setAttribute("href", encodeURI(csvContent));
    link.setAttribute("download", `Staff_Appraisals_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    if (!appraisals.data.length) return;
    let text = "Staff Name\tPeriod\tRating\tEvaluator\tDate\n";
    appraisals.data.forEach(item => {
      text += `${item.staff?.first_name || ''} ${item.staff?.last_name || ''}\t${item.period}\t${item.rating}\t${item.evaluator?.name}\t${item.appraisal_date}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Performance & Appraisals - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Performance & Appraisals</h1>
            <p className="text-sm text-slate-500 mt-1">শিক্ষক ও কর্মচারীদের কাজের মূল্যায়ন ও পারফরম্যান্স রিভিউ করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="star" className="w-4 h-4" /> Evaluate Staff
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Appraisal Period</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Performance Rating</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Evaluator &amp; Date</th>
                  ...
                  appraisals.data.map((item, index) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4 text-sm font-semibold text-slate-500">
                        {(appraisals.from ?? 1) + index}
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-900 block">{item.staff?.first_name} {item.staff?.last_name || ''}</span>
                        <span className="text-xs text-slate-500 font-medium block mt-0.5">ID: {item.staff?.staff_id_no} | {item.staff?.designation?.name || 'Staff'}</span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        ...
                        {item.remarks && (
                          <p className="text-xs text-slate-500 max-w-xs truncate mt-1 italic" title={item.remarks}>
                            "{item.remarks}"
                          </p>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-bold text-slate-700 block">{new Date(item.appraisal_date).toLocaleDateString('en-GB')}</span>
                        <span className="text-[11px] text-slate-400 block mt-0.5">By: {item.evaluator?.name}</span>
                      </td>
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setIsFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Appraisal">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Appraisal">
