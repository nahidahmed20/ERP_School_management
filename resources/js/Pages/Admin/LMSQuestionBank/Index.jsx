...
export default function Index({ questions, classes, subjects, campuses, filters }) {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `Question_Bank_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Question\tClass\tSubject\tType\tMarks\tStatus\n";
    questions.data.forEach(item => {
      text += `${item.question}\t${item.school_class?.name || '-'}\t${item.subject?.name || '-'}\t${item.question_type}\t${item.marks}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Question Bank</h1>
            <p className="text-sm text-slate-500 mt-1">পরীক্ষার জন্য বিষয়ভিত্তিক প্রশ্নব্যাংক তৈরি ও পরিচালনা করুন।</p>
          </div>
          <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  ...
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider w-40">Class &amp; Subject</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider w-32">Type / Marks</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider w-24 text-center">Status</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right no-print w-36">Actions</th>
                </tr>
              </thead>
                ...
                {questions.data.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                      ...
                      <p className="text-sm font-semibold text-slate-600">কোনো প্রশ্ন পাওয়া যায়নি</p>
                      <p className="text-xs text-slate-400 mt-1">Try adjusting filters or add a new question</p>
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="text-sm font-semibold text-slate-800 line-clamp-2" title={item.question}>
                            {item.question}
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-bold text-slate-900 block">{item.school_class?.name}</span>
                        <span className="text-xs text-slate-500 font-medium block mt-0.5">{item.subject?.name}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="inline-flex px-2 py-0.5 rounded text-[10px] font-bold tracking-wide uppercase bg-sky-50 text-sky-700 border border-sky-200 mb-1">
                          {item.question_type}
                        </span>
                        <div className="text-sm font-black text-emerald-700 font-mono">Marks: {item.marks}</div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => setViewingItem(item)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="View Question">
                            <Icon name="eye" className="w-4 h-4" />
                          </button>
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Question">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Question">
