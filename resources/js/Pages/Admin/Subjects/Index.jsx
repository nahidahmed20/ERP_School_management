...
export default function Index({ subjects, campuses, filters }) {
  ...
  const exportToCSV = () => {
    if (!subjects.data.length) return Swal.fire({ icon: 'warning', title: 'No Data!', text: 'Export করার মতো কোনো ডেটা নেই।' });
    const headers = ['Subject Name', 'Subject Code', 'Type', 'Status'];
    const rows = subjects.data.map(item => [
      item.name || 'N/A', 
      item.code || 'N/A', 
      item.type || 'N/A', 
    ...
    link.setAttribute("download", `Subjects_Directory_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  const copyToClipboard = () => {
    ...
    let text = "Subject Name\tSubject Code\tType\tStatus\n";
    subjects.data.forEach(item => {
      text += `${item.name || 'N/A'}\t${item.code || 'N/A'}\t${item.type || 'N/A'}\t${item.is_active ? 'Active' : 'Inactive'}\n`;
    });
  ...
  return (
    <AuthenticatedLayout>
      ...
      <div className="print-title">Subjects Directory - {new Date().toLocaleDateString('en-GB')}</div>
      ...
      <div className="w-full space-y-6 sm:px-6 lg:px-8 py-8 no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight mt-1">Subjects</h1>
            <p className="text-sm text-slate-500 mt-1">স্কুল বা কলেজের সব বিষয় বা সাবজেক্ট এখান থেকে পরিচালনা করুন।</p>
          </div>
          <button
          ...
          >
            <Icon name="plus" className="w-4 h-4" /> Add Subject
          </button>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/50">
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Subject Name</th>
                  <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Subject Code</th>
                ...
                {subjects.data.length === 0 ? (
                  ...
                  subjects.data.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {item.code ? (
                          <code className="text-xs font-mono font-bold bg-slate-100 text-slate-700 px-2 py-1 rounded border border-slate-200">
                            {item.code}
                          </code>
                        ) : '—'}
                      </td>
                      <td className="px-6 py-4">
                        <span className="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold tracking-wide uppercase bg-indigo-50 text-indigo-700 border border-indigo-200">
                          {item.type}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right no-print">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingItem(item); setFormOpen(true); }} className="p-2 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Subject">
                            <Icon name="edit" className="w-4 h-4" />
                          </button>
                          <button onClick={() => setDeletingItem(item)} className="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete Subject">
      ...
      {deletingItem && (
        <ConfirmDeleteModal
          ...
          onConfirm={() => {
            router.delete(route('admin.subjects.destroy', deletingItem.id), {
