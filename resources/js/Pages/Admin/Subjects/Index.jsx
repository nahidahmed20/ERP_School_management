      ...
      header={
        <div className="page-head">
          <div>
            ...
            <h1>Subjects</h1>
            <p className="desc">স্কুল বা কলেজের সব বিষয় বা সাবজেক্ট এখান থেকে পরিচালনা করুন।</p>
          </div>
          ...
          <table className="mm-table">
            ...
            <tbody>
              {subjects.data.length === 0 && (
                <tr><td colSpan={5} className="mm-empty">কোনো সাবজেক্ট পাওয়া যায়নি।</td></tr>
              )}
              {subjects.data.map((item) => (
                <tr key={item.id}>
                  <td>
                    <div className="mm-label-cell">
                      <Icon name="book" className="mm-row-icon" />
                      <span style={{ fontWeight: 500 }}>{item.name}</span>
                    </div>
                  </td>
                  <td>{item.code ? <code>{item.code}</code> : '—'}</td>
                  <td>
                    <span className="badge" style={{ padding: '4px 8px', background: '#e0e7ff', color: '#3730a3', borderRadius: '4px', fontSize: '12px' }}>
                      {item.type}
                    </span>
                  </td>
                  <td>
